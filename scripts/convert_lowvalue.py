...
src_dir = 'pcl5/low-value'
dst_dir = 'pcl5_new/low-value'
os.makedirs(dst_dir, exist_ok=True)

with open('/tmp/lowvalue_files.txt') as f:
    files = [line.strip() for line in f if line.strip()]

for fname in files:
    orig_path = os.path.join(src_dir, fname)
    base = fname[len('test_pcl5_lowvaluenew_'):-3]
    new_fname = f'test_when_printing_pcl5_lowvaluenew_{base}.py'
    ...
    for line in metadata.splitlines():
        ...
        if re.search(r'\+name:', line):
            ...
            line = f"{indent}+name:TestWhenPrintingJPEGFile::test_when_using_pcl5_lowvaluenew_{base}_file_then_succeeds"
...
class TestWhenPrintingJPEGFile(TestWhenPrinting):
    ...
    {metadata_new}
    """
    def test_when_using_pcl5_lowvaluenew_{base}_file_then_succeeds(self):
{body}
''')
