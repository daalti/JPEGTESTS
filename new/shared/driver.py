...
from dunetuf.ui.new.shared.locator_base import LocatorCombiner
from .locator_resolver import LocatorResolver, resolve_to_selector
...
from dunetuf.ui.new.shared.locator_base import Locator  # type: ignore
...
class QmlDriver:
    ...
    def __init__(
        ...
        locator_resolver: Optional[LocatorResolver] = None
    ) -> None:
    ...
    # Selector handling
    def _normalize_selector(self, target: Union[str, LocatorType]) -> str:
        """Convert target to selector string using centralized resolver."""
        try:
            # Use resolver for consistent handling
            resolved = self._locator_resolver.resolve_to_selector_string(target)
            if isinstance(resolved, list):
                raise ValueError("Single selector expected, got list")
            return resolved
    ...
    def find_nested_element(
        self, 
        target: Union[str, LocatorType, List[Union[str, LocatorType]]], 
        index: int = 0
    ) -> QmlElement:
        """Find nested element with simplified locator handling."""
        if not isinstance(target, List):
            return self.find(target, index)
        
        if not target:
            raise ValueError("Target list cannot be empty")

        # Use resolver to handle the list
        resolved_selectors = self._locator_resolver.resolve_to_selector_string(target)
        if not isinstance(resolved_selectors, list):
            raise ValueError("Expected list of selectors")
        
        combined_selector = " ".join(resolved_selectors)
        self._logger.debug(f"Combined selector for nested search: {combined_selector}")

        return self.find(combined_selector, index)
    ...
    def wait_for_idle(self,
                      stable_ms: float = 500,
                      timeout: Optional[float] = QmlDriverConfig.wait_timeout,
                      probe_interval: float = 0.2) -> None:
        """
        Wait for the QML scene to be stable (no changes) for stable_ms milliseconds.
        Avoids overwhelming the UI thread by using a small probe interval.
        
        Args:
            stable_ms: Minimum time in milliseconds the scene must remain stable
            timeout: Maximum time to wait for stability
            probe_interval: Time between stability checks
            
        Raises:
            QmlTimeout: If UI doesn't reach stable state within timeout
        """
        timeout = self._apply_timeout_factor(timeout or self.config.until_timeout)
        start = time.time()
        stable_since = None
        ...
        def scene_signature() -> str:
            try:
                # Read minimal data: compact format and truncate to avoid overloading
                txt = self._request("dumpQmlScene", {"format": "text", "style": "compact"})
                txt = txt if isinstance(txt, str) else str(txt)
                # Hash only a portion for efficiency
                return hashlib.sha1(txt[:4096].encode("utf-8", "ignore")).hexdigest()
            except Exception:
                # If it fails, return something changing to keep waiting
        ...
        while True:
            ...
            if time.time() - start > timeout:
                raise QmlTimeout(f"UI did not reach stable state within {timeout:.2f}s")

            time.sleep(probe_interval)

    def wait_for_visible_stable(self,
                                target: Union[str, LocatorType],
                                index: int = 0,
                                min_stable_ms: float = 200,
                                timeout: Optional[float] = None) -> QmlElement:
        
        timeout = self._apply_timeout_factor(timeout or self.config.wait_timeout)
        start = time.time()
        stable_since = None
        el: Optional[QmlElement] = None

        while True:
            try:
                el = self.find(target, index)
                vis = el.is_visible
                ena = el.is_enabled
                opa = float(el.get("opacity", 1.0))
                if vis and ena and opa >= 0.95:
                    if stable_since is None:
                        stable_since = time.time()
                    elif (time.time() - stable_since) * 1000 >= min_stable_ms:
                        return el
                else:
                    stable_since = None
            except QmlElementNotFound:
            ...
            if time.time() - start > timeout:
                raise QmlTimeout(f"{target} not visible/stable after {timeout:.2f}s")
    ...
    def scroll_to_element(
        ...
        scroll_direction: Literal["vertical", "horizontal"] = "vertical",  # New parameter
        ...
        Args:
            ...
            scroll_increment: Increment for position-based scrolling (0.0 to 1.0)
            scroll_direction: Scroll direction ("vertical" or "horizontal") 
            max_scroll_attempts: Maximum number of scroll attempts
            scroll_step: Pixels to scroll each time for wheel scrolling
        ...
        """
        self._logger.debug(f"Scrolling to find element: {target_element}, direction: {scroll_direction}")
        ...
        for attempt in range(max_scroll_attempts):
            ...
            if scrollbar:
                # Try position-based scrolling first
                try:
                    position = min((attempt + 1) * scroll_increment, 1.0)
                    self.scroll_to_position(position, scrollbar, scroll_direction)
                except Exception as e:
                    self._logger.debug(f"Position-based scrolling failed: {e}, falling back to wheel")
                    ...
                    if scroll_direction == "horizontal":
                        ...
                        wheel_x = 0
                        wheel_y = scroll_step
                    scrollbar_element.wheel(wheel_x, wheel_y)
