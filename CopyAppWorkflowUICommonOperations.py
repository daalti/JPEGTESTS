...
class CopyAppWorkflowUICommonOperations(ICopyAppUIOperations):
    ...
    def verify_copy_mediasize_selected_option(self, net, setting, setting_value, screen_id = CopyAppWorkflowObjectIds.view_copySettingsView):
        """
        This method compares the selected setting string with the expected string from string id
        Args:
            UI should be in Copy Landing view
        ...
        time.sleep(2)
        # Can add other options when need
        if (setting.lower() == "original"):
            setting_id = CopyAppWorkflowObjectIds.view_originalSetting
            cstring_id = self.original_dict[setting_value][0]
            row_object_id = CopyAppWorkflowObjectIds.list_copySettings_originalSize
        elif (setting.lower() == "paper"):
            setting_id = CopyAppWorkflowObjectIds.view_paperSetting
            cstring_id = self.paper_dict[setting_value][0]
            row_object_id = CopyAppWorkflowObjectIds.list_copySettings_paperSelection
        elif (setting.lower() == "paper_tray"):
            setting_id = CopyAppWorkflowObjectIds.view_paperSetting
            cstring_id = self.paper_tray_option_dict[setting_value][0]
            row_object_id = CopyAppWorkflowObjectIds.list_copySettings_paperSelection
        elif (setting.lower() == "paper_type"):
            setting_id = CopyAppWorkflowObjectIds.view_paperSetting
            cstring_id = self.paper_type_option_dict[setting_value][0]
            row_object_id = CopyAppWorkflowObjectIds.list_copySettings_paperSelection
        else:
            assert False, "Setting not existing"

        self.workflow_common_operations.goto_item(row_object_id, screen_id, select_option=False, scrollbar_objectname=CopyAppWorkflowObjectIds.copy_options_scrollbar)

        ui_setting_string = self.spice.query_item(setting_id + " SpiceText[visible=true]")["text"] 
        if setting == "paper":
            ui_setting_string = ui_setting_string.split(",")[0]
        if setting == "paper_tray":
            ui_setting_string = ui_setting_string.split(",")[1].strip()
        if setting == "paper_type":
            ui_setting_string = ui_setting_string.split(",")[2].strip()
    ...
    def verify_copy_paper_selection_option(self, net, setting, setting_value):
        ...
        if setting == "paper_size":
            expected_string_id = self.paper_dict[copy_paper_size_option_dict()[setting_value]][0]
            actual_string_id = CopyAppWorkflowObjectIds.view_copySettings_paperSize
        elif setting == "paper_tray":
            expected_string_id = self.paper_tray_option_dict[copy_paper_tray_option_dict()[setting_value]][0]
            actual_string_id = CopyAppWorkflowObjectIds.combo_copySettings_paperTray
        elif setting == "paper_type":
            expected_string_id = self.paper_type_option_dict[copy_paper_type_option_dict()[setting_value]][0]
            actual_string_id = CopyAppWorkflowObjectIds.view_copySettings_paperType
        ...
        assert  actual_string_id == expected_string, "paper selection settings value mismatch"
    ...
    def start_preview(self):
        ...
        '''
        self.spice.wait_for(CopyAppWorkflowObjectIds.button_preview)
        copy_expand_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_preview)
        copy_expand_button.mouse_click()

    def wait_for_copy_landingview_to_load(self):
        '''
        Ui Should be in CopyLanding
        Wait for copy landing view to load completely
        Ui Flow: Copy app -> Copy Landing
        '''
        copy_landing_view = self.spice.wait_for(CopyAppWorkflowObjectIds.view_copyScreen, timeout =9.0)
        self.spice.wait_until(lambda: copy_landing_view["visible"] == True, timeout = 15.0)
        copy_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_startCopy, timeout =9.0)
        self.spice.wait_until(lambda: copy_button["enabled"] == True, timeout = 15.0)
        self.spice.wait_until(lambda: copy_button["visible"] == True, timeout = 15.0)
        copy_all_options_button = self.spice.wait_for(CopyAppWorkflowObjectIds.view_copyScreen + " " + CopyAppWorkflowObjectIds.button_copyMoreOptions, timeout = 10)
        self.spice.wait_until(lambda: copy_all_options_button["enabled"] == True, timeout = 15.0)
        self.spice.wait_until(lambda: copy_all_options_button["visible"] == True, timeout = 15.0)

    def wait_for_preview_panel_to_load(self):
        '''
        Ui Should be in previewpanel
        Wait for preview panel to load completely
        Ui Flow: Copy app -> Preview panel
        '''
        fitpage_layout = self.spice.wait_for(CopyAppWorkflowObjectIds.fitapge_layout, timeout=15.0)
        self.spice.wait_for(CopyAppWorkflowObjectIds.first_preview_image, timeout =15.0)
        self.spice.wait_until(lambda: fitpage_layout["isPreviewImageAvailable"] == True)
        preview_add_page_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_add_page)
        ...
        copy_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_startCopy, timeout=15.0)
        self.spice.wait_until(lambda: copy_button["enabled"] == True, timeout = 15.0)
    ...
    def start_copy_from_secondary_panel(self, familyname = "", adfLoaded = False, prompt_for_additional_pages = False):
        '''
    ...
    def start_copy_from_preview_panel(self, familyname = "", adfLoaded = False):
        '''
        Ui Should be in previewpanel
        Click on copy button starts copy
        '''
    ...
    def cancel_copy_from_preview_panel(self):
        '''
        Ui Should be in previewpanel
        Click on cancel button discard the scanned job
        '''
        self.spice.wait_for(CopyAppWorkflowObjectIds.preview_panel)
        copy_expand_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_preview)
        copy_expand_button.mouse_click()
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.cancel_button_previewpanel)
        current_button.mouse_click()
    ...
    def refresh_preview_from_preview_panel_refresh_button(self):
        ...
        '''
        self.spice.wait_for(CopyAppWorkflowObjectIds.fitapge_layout)
        refresh_button = self.spice.wait_for(CopyAppWorkflowObjectIds.refresh_preview_button)
    ...
    def refresh_preview_from_warning_icon(self):
        ...
        '''
        self.spice.wait_for(CopyAppWorkflowObjectIds.fitapge_layout)
        preview_image_button = self.spice.wait_for(CopyAppWorkflowObjectIds.preview_image)
        preview_image_button.mouse_click()
        time.sleep(2)
        self.spice.wait_for(CopyAppWorkflowObjectIds.refresh_modal_dialog_screen)
        refresh_button = self.spice.wait_for(CopyAppWorkflowObjectIds.refresh_modal_dialog_refresh_button)
        refresh_button.mouse_click()
    
    def verify_adf_loaded_warning_modal_dialog(self):
        '''
        Ui Should be Copy App
        if preview in progress ADF warning modal dialog will be shown
        '''
        self.spice.wait_for(CopyAppWorkflowObjectIds.copy_adf_warning_modal_dialog)

    def start_copy_and_checkfor_preview(self):
        '''
        Ui Should be in CopyLanding
        Click on copy button starts copy > Preview screen is validated
        '''
        self.spice.wait_for(CopyAppWorkflowObjectIds.view_copyScreen, timeout =9.0)
        time.sleep(2)
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_startCopy)
    ...
    def verify_prepreview_screen_string_mdf(self,udw,net):
        udw.mainApp.ScanMedia.loadMedia("MDF")
        ui_string = self.spice.query_item(CopyAppWorkflowObjectIds.pre_preview_layout +" "+ CopyAppWorkflowObjectIds.pre_preview_content +" #contentItem")["text"]
        expected_string = self.spice.common_operations.get_expected_translation_str_by_str_id(net,'cCopyToStartJob')
        assert ui_string == expected_string, "String mismatch"
        udw.mainApp.ScanMedia.unloadMedia("MDF")
        # wait for the pre-preview message to be updated; currently we don't have any other way to wait for it
        time.sleep(2)
        ui_string =self.spice.query_item(CopyAppWorkflowObjectIds.pre_preview_layout +" "+ CopyAppWorkflowObjectIds.pre_preview_content +" #contentItem")["text"]
        expected_string = self.spice.common_operations.get_expected_translation_str_by_str_id(net,'cCopyOptionsToScan')
        assert ui_string == expected_string, "String mismatch"

    def verify_prepreview_screen_string_adf(self,udw,net, locale: str = "en"):
        # Load ADF media to validate the prepreview string id for ADF
        udw.mainApp.ScanMedia.loadMedia("ADF")
        adf_preview_ui_string = self.spice.query_item(CopyAppWorkflowObjectIds.pre_preview_layout +" "+ CopyAppWorkflowObjectIds.pre_preview_content +" #contentItem")["text"]
        expected_string = str(LocalizationHelper.get_string_translation(net, "cOriginalDetectedCopy", locale)) + "\n" + str(LocalizationHelper.get_string_translation(net, "cPreviewUnavailableADF", locale))
        assert adf_preview_ui_string == expected_string, "String mismatch"

        # Load Glass/Flatbed media to validate the prepreview string id for Glass/Flatbed
        udw.mainApp.ScanMedia.unloadMedia("ADF")
        glass_preview_ui_string =self.spice.query_item(CopyAppWorkflowObjectIds.pre_preview_layout +" "+ CopyAppWorkflowObjectIds.pre_preview_content +" #contentItem")["text"]
        expected_string = str(LocalizationHelper.get_string_translation(net, ["cLoadingOriginalDocument", str(LocalizationHelper.get_string_translation(net, "cPreviewLabel", locale))], locale))
        assert glass_preview_ui_string == expected_string, "String mismatch"

        # Loading ADF media to ensure the next test should not affect upon this testcase completion
        udw.mainApp.ScanMedia.loadMedia("ADF")
    
    def verify_prepreview_enabled_screen_string(self, udw, net):
        ...
        current_string_id = self.spice.query_item(CopyAppWorkflowObjectIds.pre_preview_layout +" "+ CopyAppWorkflowObjectIds.pre_preview_content +" #contentItem")["text"]
        assert current_string_id != "", "Error: String ID Not found"
        expected_string_id = self.spice.common_operations.get_expected_translation_str_by_str_id(net,CopyAppWorkflowObjectIds.prepreview_enabled_string_id)
    ...
    def verify_prepreview_disabled_screen_string(self, udw, net):
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_button)
        self.spice.wait_until(lambda: current_button['enabled'] == True, 10.0)
        current_string_id = self.spice.query_item(CopyAppWorkflowObjectIds.pre_preview_layout +" "+ CopyAppWorkflowObjectIds.pre_preview_content +" #contentItem")["text"]
        assert current_string_id != "", "Error: String ID Not found"
        expected_string_id = self.spice.common_operations.get_expected_translation_str_by_str_id(net,CopyAppWorkflowObjectIds.prepreview_disabled_string_id)
        assert expected_string_id != "", "Error: String ID Not not trnaslated"
        assert current_string_id == expected_string_id, f"Error: String mismatch {current_string_id} != {expected_string_id}"

    def cancel_copy_job(self):
        '''
        Ui Should be in CopyLanding
        Click on cancelButton Cancels the copy job
        '''
        self.spice.wait_for(CopyAppWorkflowObjectIds.view_copyScreen, timeout =10.0)
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_cancel_button)
        self.spice.wait_until(lambda: current_button["enabled"] is True, 20)
        self.spice.wait_until(lambda: current_button["visible"] is True, 20)
        current_button.mouse_click()

    def is_media_loaded(self,media_id, udw):
        """
        Start load media
        Args:
            media_id: Id of the media to launch load
        Returns:
            boolean: value to tell if udw command was executed ok
        """
        if False == udw.mainApp.ScanMedia.isMediaLoaded(media_id):
            return True
        else:
            return False 
    ...
    def check_preview_button_visible(self):
        ...
        '''
        self.spice.wait_for(CopyAppWorkflowObjectIds.preview_panel)
        current_button = self.spice.query_item(CopyAppWorkflowObjectIds.button_preview)
    ...
    def media_mismatch_flow(self):
        ...
        '''
        try:
            if (self.spice.query_item(CopyAppWorkflowObjectIds.media_mismatch_flow)["visible"] == True):
                try:
                    current_button = self.spice.query_item(CopyAppWorkflowObjectIds.media_mismatch_alert_ok)
                except Exception:
                    print("No mismatch pop-up displayed")
                    current_button = self.spice.query_item(CopyAppWorkflowObjectIds.media_mismatch_alert_print)
                current_button.mouse_click()
                time.sleep(3)
        except Exception:
            print("No mismatch pop-up displayed")

    def copy_duplex_continue(self, familyname = "", sided="1_1_sided", pages_per_sheet="1", operation_type="Continue"):
        '''
        UI should be in Media size mismatch alert popup.
        Press Ok button from the media mismatch alert
        '''
        if sided == None:
            sided = "1_1_sided"
        if pages_per_sheet == None:
            pages_per_sheet = "1"
        repeat = 0

        if familyname == "enterprise":
            if sided != "1_1_sided" or pages_per_sheet != "1":
                if sided == "2_1_sided" or sided == "2_2_sided":
                    if operation_type.lower() == "cancel":
                        self.duplex_add_page_pop_up_cancel()
                        self.add_page_pop_up_cancel_yes()
                    else:
                        self.duplex_add_page_pop_up_finish()
                else:
                    if operation_type.lower() == "cancel":
                        self.add_page_pop_up_cancel()
                        self.add_page_pop_up_cancel_yes()
                    else:
                        self.add_page_pop_up_done()
                        
            else:
                logging.info("If 1-1 Side or pagespert sheet is 1, Alert window will not appear.")
                pass
        elif familyname == "homepro":
            if sided != "1_1_sided" or pages_per_sheet != "1":
                if (sided == "1_2_sided" or sided == "2_2_sided") and pages_per_sheet == "2":
                    repeat = 3
                else:
                    repeat = 1
                self.spice.wait_for(CopyAppWorkflowObjectIds.copy_2sided_prompt, timeout = 15.0)
                if operation_type.lower() == "cancel":
                    self.cancel_copy_2sided_operation(familyname)
                else:
                    for i in range(0, repeat):
                        logging.error("Click Continue button on 2-Sided Copying screen")
                        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.duplex_copy_continue)
                        current_button.mouse_click()
                        time.sleep(2)   # sleep to remove prompt and then wait for same Prompt to appear again 
            else:
                logging.info("If 1-1 Side or pagespert sheet is 1, Alert window will not appear.")
                pass
    
    def verify_copy_constrained_message(self,net):
        '''
        Checking the constraint Message text and close the modal
        '''
        ui_string = self.spice.wait_for(CopyAppWorkflowObjectIds.constraint_string_msg)["message"]
        expected_string = self.spice.common_operations.get_expected_translation_str_by_str_id(net,'cInsertPageInScanner')
        assert ui_string == expected_string, "String mismatch"
        closeModal = self.spice.wait_for(CopyAppWorkflowObjectIds.ok_button)
        closeModal.mouse_click()

    def cancel_copy_from_preview_panel_launch_from_widget(self):
        '''
        Ui Should be in CopyLanding
        Click on cancelButton Cancels the copy job
        '''
    ...
    def get_output_scale_options(self):
        ...
        self.spice.wait_for(CopyAppWorkflowObjectIds.options_view)

        logging.info(f"To get actual str of " + CopyAppWorkflowObjectIds.option_output_scale_button_locator)
        item = self.spice.wait_for(CopyAppWorkflowObjectIds.option_output_scale_button_locator)
        self.spice.wait_until(lambda: item["visible"] is True, 20)
        temp = self.spice.query_item(CopyAppWorkflowObjectIds.option_output_scale_button_locator + " #contentItem")
        current_output_scale_option = self.spice.query_item(CopyAppWorkflowObjectIds.option_output_scale_button_locator + " #contentItem")["text"]
        logging.info("Current output scale settings is: " + current_output_scale_option)
        return current_output_scale_option

    def set_output_scale_options(self, net, output_scale_options="none", detail_option=None):
        """
        ...
        @return:
        """
        if output_scale_options == "loaded_paper":
            loaded_paper_item = self.spice.query_item(CopyAppWorkflowObjectIds.option_output_scale_set_loaded_paper)
            loaded_paper_item.mouse_click()
            if detail_option:
                detail_item = self.spice.query_item(CopyAppWorkflowObjectIds.option_output_scale_set_loaded_paper_detail)
                detail_item.mouse_click()
                if detail_option == "roll":
                    roll_item = self.spice.query_item(CopyAppWorkflowObjectIds.option_output_scale_set_loaded_paper_detail_roll)
                    roll_item.mouse_click()
                elif detail_option == "tray":
                    tray_item = self.spice.query_item(CopyAppWorkflowObjectIds.option_output_scale_set_loaded_paper_detail_tray)
                    tray_item.mouse_click()

        elif output_scale_options == "standard_sizes":
            standard_sizes_item = self.spice.query_item(CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes)
            standard_sizes_item.mouse_click()
            if detail_option:
                detail_item = self.spice.query_item(CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail)
                detail_item.mouse_click()

                if detail_option == "a0":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_a0
                elif detail_option == "a1":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_a1
                elif detail_option == "a2":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_a2
                elif detail_option == "a3":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_a3
                elif detail_option == "a4":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_a4
                elif detail_option == "b1":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_b1
                elif detail_option == "b2":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_b2
                elif detail_option == "b3":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_b3
                elif detail_option == "b4":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_b4
                elif detail_option == "letter":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_letter
                elif detail_option == "ledger":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_ledger
                elif detail_option == "ansi_c":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_ansi_c
                elif detail_option == "ansi_d":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_ansi_d
                elif detail_option == "ansi_e":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_ansi_e
                elif detail_option == "arch_a":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_arch_a
                elif detail_option == "arch_b":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_arch_b
                elif detail_option == "arch_c":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_arch_c
                elif detail_option == "arch_d":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_arch_d
                elif detail_option == "arch_e":
                    detail_option_id = CopyAppWorkflowObjectIds.option_output_scale_set_standard_sizes_detail_arch_e

                self.workflow_common_operations.goto_item(detail_option_id, CopyAppWorkflowObjectIds.standard_sizes_optionView, scrollbar_objectname = CopyAppWorkflowObjectIds.standard_sizes_scrollbar)
        elif output_scale_options == "none":
            none_item = self.spice.query_item(CopyAppWorkflowObjectIds.radio_outputScale_none)
            none_item.mouse_click()
        

    def check_spec_on_usb_print_options_output_scale(self, net, output_scale=None):
        """
        ...
        time.sleep(1)
        self.workflow_common_operations.verify_string(net, CopyAppWorkflowObjectIds.output_scale_str_id, CopyAppWorkflowObjectIds.option_output_scale_menu_loctor)

        if(output_scale):
            logging.info("check output scale options spec")
            assert self.get_output_scale_options() == output_scale

    def check_duplex_support(self,cdm):
        response = cdm.get("cdm/scan/v1/status")
        if(response['adf']['duplexSupported'] == 'false'):
            return False
        return True

    def enable_duplex_supported(self,cdm,udw):
        udw.mainApp.ScanMedia.unloadMedia("ADF")
        udw.mainApp.ScanCapabilities.setHasDuplexSupport(True)
        result = self.check_duplex_support(cdm)
        assert result == True
        udw.mainApp.ScanMedia.loadMedia("ADF")

    def disable_duplex_supported(self,cdm,udw):
        udw.mainApp.ScanMedia.unloadMedia("ADF")
    ...
    def check_original_size_option(self, option):
        ...
        copy_originalSize_options_dict = {
            ...
            "A5": {
                "item_id": CopyAppWorkflowObjectIds.radio_originalSize_iso_a5_148x210mm,
                "row_id": CopyAppWorkflowObjectIds.row_original_size_iso_a5_148x210mm
                },
            "B5": {
                "item_id": CopyAppWorkflowObjectIds.radio_originalSize_jis_b5_182x257mm,
                "row_id": CopyAppWorkflowObjectIds.row_original_size_jis_b5_182x257mm
                },                   
            "Letter": {
                "item_id": CopyAppWorkflowObjectIds.radio_originalSize_na_letter_8_5x11in,
        ...
        self.workflow_common_operations.goto_item(to_select_item["row_id"], 
            CopyAppWorkflowObjectIds.view_copySettings_originalSize, 
            scrollbar_objectname=CopyAppWorkflowObjectIds.copy_originalSize_scrollbar,
            select_option=False, scrolling_value=0.05)
        
        assert self.spice.wait_for(to_select_item["item_id"], timeout = 3.0)

    def check_media_size_option(self, option):
            # # media size
        #self.go_to_paper_selection()
        #self.goto_copy_paper_size_screen()
        time.sleep(3)
        copy_paper_Size_options_dict = {
            "A4": {
                "item_id": CopyAppWorkflowObjectIds.radio_paperSize_iso_a4_210x297mm,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_iso_a4_210x297mm
                },
            "A5": {
                "item_id": CopyAppWorkflowObjectIds.radio_paperSize_iso_a5_148x210mm,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_iso_a5_148x210mm
                },
            "B5": {
                "item_id": CopyAppWorkflowObjectIds.radio_paperSize_jis_b5_182x257mm,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_jis_b5_182x257mm
                },                   
            "Letter": {
                "item_id": CopyAppWorkflowObjectIds.radio_paperSize_na_letter_8_5x11in,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_na_letter_8_5x11in
                },
            "Legal": {
                "item_id": CopyAppWorkflowObjectIds.radio_paperSize_na_legal_8_5x14in,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_na_legal_8_5x14in
                },
            "Letter_SEF": {
                "item_id": CopyAppWorkflowObjectIds.radio_paperSize_rotate_na_letter_8_5x11in,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_rotate_na_letter_8_5x11in
                },
            "A4_SEF": {
                "item_id": CopyAppWorkflowObjectIds.radio_paperSize_rotate_iso_a4_210x297mm,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_rotate_iso_a4_210x297mm
                },
            "A5_SEF": {
                "item_id": CopyAppWorkflowObjectIds.radio_paperSize_rotate_iso_a5_148x210mm,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_rotate_iso_a5_148x210mm
                },
            "B5_SEF": {
                "item_id": CopyAppWorkflowObjectIds.radio_paperSize_rotate_jis_b5_182x257mm,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_rotate_jis_b5_182x257mm
                }   
            }
        to_select_item = copy_paper_Size_options_dict.get(option)
        self.workflow_common_operations.goto_item(to_select_item["row_id"], 
    ...
    def wait_for_release_page_prompt_and_click_relasePage(self, timeout=10):
        """
        Clicks on release page button of Relasepage button
        On non-concurrent MDF products
        """
        self.spice.wait_for(CopyAppWorkflowObjectIds.copy_relase_pagePrompt,timeout)
        button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_release_pagePrompt_pagebtn)
        button.mouse_click()
        
    def wait_for_completion_screen_and_click_ok(self):
        self.spice.wait_for(CopyAppWorkflowObjectIds.copy_active_job_modal_text_locator)
        button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_active_job_modal_text_locator)
        button.mouse_click()
    ...
    def verify_prepreview_screen(self):
        ...
        """
        self.spice.wait_for(CopyAppWorkflowObjectIds.pre_preview_layout)

    def home_screen_has_lock_icon(self):
        self.spice.goto_homescreen()
        lock_icon_id = CopyAppWorkflowObjectIds.copyFolder_home_screen + " #statusIconRect SpiceLottieImageView"

        try:
            lock_icon = self.spice.wait_for(lock_icon_id)
        except:
            logging.info("Failed to find lock icon for Copy App")
            return False
    ...
    def goto_home_screen_at_copy_app(self):
        ...
        time.sleep(3)
        copy_icon = self.spice.wait_for(CopyAppWorkflowObjectIds.app_copy_home_screen + " MouseArea")
    ...
    def set_detailed_original_media(self, value):
        ...
        if value == "white":
            self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_list,menu_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_white,top_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option,select_option = False)
            current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option_white)
            current_button.mouse_click()
            select_original_paper_type = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option+' #contentItem')
            self.spice.wait_until(lambda: select_original_paper_type["visible"] == True, timeout = 10.0)
            logging.info("The selected original paper type is " + select_original_paper_type['text'])
            assert select_original_paper_type['text'] == 'White'
        if value == "photo":
            self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_list,menu_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_photo,top_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option,select_option = False)
            current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option_photo)
            current_button.mouse_click()
            select_original_paper_type = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option+' #contentItem')
            self.spice.wait_until(lambda: select_original_paper_type["visible"] == True, timeout = 10.0)
            logging.info("The selected original paper type is " + select_original_paper_type['text'])
            assert select_original_paper_type['text'] == 'Photo'
        if value == "translucent":
            self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_list,menu_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_translucent,top_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option,select_option = False)
            current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option_translucent)
            current_button.mouse_click()
            select_original_paper_type = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option+' #contentItem')
            self.spice.wait_until(lambda: select_original_paper_type["visible"] == True, timeout = 10.0)
            logging.info("The selected original paper type is " + select_original_paper_type['text'])
            assert select_original_paper_type['text'] == 'Translucent'
        if value == "old":
            self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_list,menu_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_oldRecycled,top_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option,select_option = False)
            current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option_oldRecycled)
            current_button.mouse_click()
            select_original_paper_type = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option+' #contentItem')
            self.spice.wait_until(lambda: select_original_paper_type["visible"] == True, timeout = 10.0)
            logging.info("The selected original paper type is " + select_original_paper_type['text'])
            assert select_original_paper_type['text'] == 'Old'
        if value == "blueprint":
            self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_list,menu_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_blueprints,top_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option,select_option = False)
            current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option_blueprints)
            current_button.mouse_click()
            select_original_paper_type = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option+' #contentItem')
            self.spice.wait_until(lambda: select_original_paper_type["visible"] == True, timeout = 10.0)
            logging.info("The selected original paper type is " + select_original_paper_type['text'])
            assert select_original_paper_type['text'] == 'Blueprint'
        if value == "ammonia_old_blueprint":
            self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_list,menu_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option_blueprints,top_item_id=CopyAppWorkflowObjectIds.setings_originalPaper_option,select_option = False)
            current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option_darkBlueprints)
            current_button.mouse_click()
            select_original_paper_type = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option+' #contentItem')
            self.spice.wait_until(lambda: select_original_paper_type["visible"] == True, timeout = 10.0)
            logging.info("The selected original paper type is " + select_original_paper_type['text'])
            assert select_original_paper_type['text'] == 'Ammonia (Old) Blueprint'
        if value == "oldRecycled":
            current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option_oldRecycled)
            current_button.mouse_click()
        if value == "darkBlueprints":
            current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.setings_originalPaper_option_darkBlueprints)
            current_button.mouse_click()
    ...
    def goto_copywidget_option_landingview_fromhomescreen(self):
        ...
        copywidget_option_button.mouse_click(0, 10)

    def select_no_of_copies_for_constraintMessage(self):
        """
        Purpose: Selects Copy option in copy screen and waits for copy successful screen
        Ui Flow: Copy screen -> Copy
        :return: None
        """
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_copyScreen)
        self.workflow_common_operations.scroll_to_position_vertical(0.2 , CopyAppWorkflowObjectIds.vertical_layout_scrollbar)
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.spinBox_numberOfCopies_plus)
        current_button.mouse_click()
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_constraint_message)
        assert self.spice.wait_for("#constraintDescription")
        textContent = self.spice.wait_for("#constraintDescription #textColumn #contentItem")   
    ...
    def copy_job_ticket_general_method(self,loadmedia:str, copy_path, copy_settings: dict, udw, net, print_emulation=None, familyname="", scan_emulation=None):
        ...
        self.configuration = Configuration(self.cdm)
        self.tray = TrayHandler(self.udw, self.cdm)
        self.media = MediaHandler(self.udw, self.cdm)
        device_identity = self.cdm.get_raw(self.cdm.SYSTEM_IDENTITY)
        pe_platform = self.udw.mainApp.execute("ConnectorDriver PUB_getPrintEnginePlatform")
        ...
        # Color mode option is not supported copy settings in UI or EWS page for Mono printer.
        if not self.cdm.device_feature_cdm.is_color_supported():
            copy_settings["colorMode"] = None
        ...
        settings = {
                ...
                '2sidedformat_flip': copy_settings.get('2sidedformat_flip', None),
                ...
                'background_color_removal' : copy_settings.get('background_color_removal', None),
                'background_noise_removal' : copy_settings.get('background_noise_removal', None),
                'auto_tone' : copy_settings.get('auto_tone', None),
                'auto_tone_level' : copy_settings.get('auto_tone_level', None),
                'auto_paper_color_removal' : copy_settings.get('auto_paper_color_removal', None),
                'auto_paper_color_removal_level' : copy_settings.get('auto_paper_color_removal_level', None),
                'long_original':copy_settings.get('long_original', None),
                'originalPaperType': copy_settings.get('originalPaperType', None),
                'paper_size': copy_settings.get('paper_size', None),
                'edge_to_edge_output' : copy_settings.get('edge_to_edge_output', None),
                'paper_selection': copy_settings.get('paper_selection', None)
            }

        logging.info("load media")
        
        if loadmedia!=None:
            if device_identity.json()["managementProfile"] == "mpLaserEnterprise" and int(pe_platform) == 3 and scan_emulation != None:
                if loadmedia == 'ADF' or 'MDF':
                    logging.info("load ADF")
                    scan_emulation.media.load_media(loadmedia)
        ...
        if copy_path!=None:
            ...
            if copy_path=='MenuCopyDocumnetSettingsPage':
                logging.info("copy from Menu Copy app settings page")
                self.goto_copy()
                self.wait_for_copy_landingview_to_load()
                if(copy_settings.__len__() != 0):
                    self.goto_copy_options_list()
                ...
                self.goto_copy_from_copyapp_at_home_screen()
                self.wait_for_copy_landingview_to_load()
                if(copy_settings.__len__() != 0):
                    self.goto_copy_options_list()
                ...
                if(copy_app_exist == True):
                    ...
                    self.workflow_common_operations.scroll_to_position_horizontal(0.3)
                    self.goto_idcopy_fromhomescreen()
                if(copy_settings.__len__() != 0):
                    self.goto_copy_options_list_from_idcopy()
        if(copy_settings.__len__() != 0):
            ...
            if settings['size'] != None:
                ...
                self.check_original_size_value(settings['size'], net)

            if settings['contentType'] != None:
                self.select_content_type(settings['contentType'])
            if settings['originalPaperType'] != None:
            ...
            if settings['2sidedformat_flip']!= None:
                self.set_copy_2sided_flip_up_options(settings['2sidedformat_flip'])            
            ...
            if settings['background_color_removal'] != None:
                self.goto_background_color_removal_settings()
                self.set_copy_settings_background_color_removal(settings['background_color_removal'])
            if settings['background_noise_removal'] != None:
                self.goto_background_noise_removal_settings()
                self.set_scan_settings_background_noise_removal(settings['background_noise_removal'])
            if settings['edge_to_edge_output'] != None:
                self.goto_edge_to_edge_output_settings()
                self.set_scan_settings_edge_to_edge_output(settings['edge_to_edge_output'])
            if settings['long_original'] != None:
            ...
            if settings['paper_selection'] != None:
                ...
                if paper_size:
                    ...
                    self.verify_paper_size_value_under_paper_selection(net, paper_size)
                ...
                if paper_type:
                    ...
                    self.check_paper_type_value(paper_type, net)
                self.go_back_to_setting_from_paper_selection()
            if settings['auto_tone'] != None:
                self.goto_auto_tone_option()
                self.set_copy_settings_auto_tone(settings['auto_tone'], settings['auto_tone_level'])
            if settings['auto_paper_color_removal'] != None:
                self.goto_auto_paper_color_removal_option()
                self.set_copy_settings_auto_paper_color_removal(settings['auto_paper_color_removal'], settings['auto_paper_color_removal_level'])
        ...
        if scan_emulation!=None:
            adfLoaded = scan_emulation.media.is_media_loaded('ADF')
        else: 
        ...
        if copy_path in ['IDCardMenuPage', 'IDCardLandingPage']:
           self.id_card_copy.start_copy()
            ...
            self.start_copy(familyname = familyname, adfLoaded=adfLoaded, sided=settings['sides'], pages_per_sheet=settings['pagesPerSheet'], loadmedia=loadmedia)
            time.sleep(3)
            if loadmedia == 'MDF':
                self.wait_for_release_page_prompt_and_click_relasePage()
            try:
                self.media.wait_for_alerts('mediaLoadFlow', timeout=6)
                
                paper_id = next((key for key, value in self.metadata_to_ui_media_sizes_map.items() if value["ui"] == settings['size']), None)
                tray_load_paper = self.tray_media_dict.get(settings['size'])
                if self.tray.is_size_supported(paper_id, 'tray-1'):
                    print_emulation.tray.empty(MediaInputIds.Tray1.name)
                    time.sleep(2)
                    print_emulation.tray.load(MediaInputIds.Tray1.name , tray_load_paper , MediaType.Plain.name , MediaOrientation.Default.name , TrayLevel.Full.name)  
                    self.tray.configure_tray('tray-1', paper_id, 'stationery')
                    logging.info("media load complete.")
            except:
                logging.info("media load alert is not appear")

    def copy_add_page_general_method(self, copy_settings: dict, udw, loadMedia:str="Flatbed", start_copy:bool=True, scan_emulation=None):
        settings = {
                'copies': copy_settings.get('copies', None),
                'pagesPerSheet': copy_settings.get('pagesPerSheet', None),
                'sides': copy_settings.get('sides', None),
                'collate' : copy_settings.get('collate', None),
                'scan_scanMode': copy_settings.get('scan_scanMode', None),
                'checkBox_for_scan_mode_prompt': copy_settings.get('checkBox_for_scan_mode_prompt', None)
            }
        ...
        if scan_emulation!=None:
            if(loadMedia == "ADF"):
                ...
                logging.info("Load media to Flatbed")
                scan_emulation.media.unload_media('ADF')
        ...
        # Go to CopyApp from Home screen
        self.goto_copy_from_copyapp_at_home_screen()
        ...
        if(copy_settings.__len__() != 0):
            ...
            if settings['collate'] != None:
                self.change_collate(settings['collate'])
            if settings['scan_scanMode'] != None:
                if settings['checkBox_for_scan_mode_prompt'] != None:
                    self.select_scan_mode_option(settings['scan_scanMode'], settings['checkBox_for_scan_mode_prompt'])
                else:
                    self.select_scan_mode_option(settings['scan_scanMode'])
    ...
    def verify_paper_size_value_under_paper_selection(self, net, option):
        ...
        """        
        self.spice.wait_for(CopyAppWorkflowObjectIds.view_copySettings_paperSelection)
        current_size = self.spice.wait_for(CopyAppWorkflowObjectIds.view_copySettings_paperSize+' #contentItem')["text"]
        logging.info(f"The current selected paper size is <{current_size}>")
        expected_string = LocalizationHelper.get_string_translation(net, self.paper_dict[option][0])
        assert current_size == expected_string, "Current paper size is inconsistent with expected string"
        logging.info(f"Current paper size is setting to <{current_size}> sucessfully")
    
    def check_paper_type_value(self, option, net):
        """
        Check if the selected value is expected
        """
        select_paper_type = self.spice.wait_for(CopyAppWorkflowObjectIds.view_copySettings_paperType+' #contentItem')
        self.spice.wait_until(lambda: select_paper_type["visible"] == True, timeout = 10.0)
        logging.info("The selected paper type is " + select_paper_type['text'])
        expected_string = LocalizationHelper.get_string_translation(net, self.paper_type_option_dict[option][0])
        assert select_paper_type['text'] == expected_string
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_copySettings_paperSelection, timeout = 9.0)
    ...
    def check_copy_sides_value(self, option, net):
        ...
        """
        select_side = self.spice.wait_for(f"{CopyAppWorkflowObjectIds.view_copySettingsView} {CopyAppWorkflowObjectIds.row_combo_copySettings_sides} {CopyAppWorkflowObjectIds.combo_copySettings_sides} #contentItem")
        self.spice.wait_until(lambda: select_side["visible"] == True, timeout = 10.0)
        logging.info("The selected side is " + select_side['text'])
        expected_string = LocalizationHelper.get_string_translation(net, self.sides_dict[option][0])
        assert select_side['text'] == expected_string
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_copySettingsView, timeout = 9.0)
    ...
    def is_original_size_visible_ui(self, original_size):
        ...
        copy_originalSize_options_dict = {
            ...
            "L (89x127 mm)": { #L (89x127 mm)
                "item_id": CopyAppWorkflowObjectIds.row_original_size_l_89x127,
                "row_id": CopyAppWorkflowObjectIds.row_media_size_l_89x127
                },
            "Envelope 6 3/4 (3.63x6.5 in.)": {
                "item_id": CopyAppWorkflowObjectIds.radio_originalSize_envelope_6,
                "row_id": CopyAppWorkflowObjectIds.row_original_size_envelope_6
        ...
        to_select_item = copy_originalSize_options_dict.get(original_size)
        is_in_view = True
        try:
            self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.view_copySettings_originalSize,menu_item_id=to_select_item["row_id"],top_item_id='#SpiceHeaderVar2',select_option=False)
            is_in_view = True
        except Exception as error:
            if str(error).find("The item is not in view of screen, so that cannot click it") == 0:
                is_in_view = False
            else:
    ...
    def wait_for_collate_limit_warning_alert(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.collate_limit_warning_alert, timeout = 50)
        logging.info("Found collateProblemWindow Alert")
        ok_button = self.spice.wait_for(CopyAppWorkflowObjectIds.collate_limit_warning_alert_ok_button)
        ok_button.mouse_click()

    def more_pages_detected_for_collate_window_continue_copy_job(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.more_pages_detected_for_collate_window, timeout = 80)
        logging.info("Found morePagesDetectedForCollateWindow Alert")
        ok_button = self.spice.wait_for(CopyAppWorkflowObjectIds.more_pages_collate_window_copy_continue_button)
        ok_button.mouse_click()
    
    def wait_for_more_pages_detected_window(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.more_pages_detected_for_collate_window, timeout = 80)
        logging.info("Found morePagesDetectedForCollateWindow Alert")


    def more_pages_detected_for_collate_window_cancel_copy_job(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.more_pages_detected_for_collate_window, timeout = 80)
        logging.info("Found morePagesDetectedForCollateWindow Alert")
        ok_button = self.spice.wait_for(CopyAppWorkflowObjectIds.more_pages_collate_window_copy_cancel_button)
        ok_button.mouse_click()

    def is_paper_type_option_available(self, option):
        self.goto_copy_paper_type_screen()
        result = self.workflow_common_operations.is_item_available(self.paper_type_option_dict[option][2],CopyAppWorkflowObjectIds.view_copySettings_paperSelection_paperType,scrollbar_objectname=CopyAppWorkflowObjectIds.copy_paperType_scrollbar)
        return result

    def is_paper_size_option_available(self, option):
        self.goto_copy_paper_size_screen()
        result = self.workflow_common_operations.is_item_available(self.paper_dict[option][1],CopyAppWorkflowObjectIds.view_copySettings_paperSelection_paperSize,scrollbar_objectname=CopyAppWorkflowObjectIds.copy_papersize_scrollbar)
        return result
    ...
    def set_content_orientation_settings(self, orientation_option):
        ...
        """
        self.workflow_common_operations.goto_item([CopyAppWorkflowObjectIds.row_object_copy_orientation, CopyAppWorkflowObjectIds.settings_copy_orientation],
                                                  CopyAppWorkflowObjectIds.view_copySettingsView, scrollbar_objectname = CopyAppWorkflowObjectIds.copy_options_scrollbar)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_copy_settings_orientation_screen)
    ...
    def is_copy_app_on_the_home_screen(self):
        exist = True
        try:
            if(self.spice.query_item(CopyAppWorkflowObjectIds.app_copy_home_screen)["visible"] == True):
                exist = True
        except:
    ...
    def add_page_pop_up_add_more(self):
        ...
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_copy_add_page_add)
        assert current_button
        current_button.mouse_click()
    
    def add_page_pop_up_scan(self):
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_copy_add_page_scan)
        assert current_button
        current_button.mouse_click()
    ...
    def add_page_pop_up_done(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_add_page_prompt_view)
        logging.info("At Add Page Pop Up")
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_copy_add_page_done, timeout = 15.0)
        assert current_button
        current_button.mouse_click()
    ...
    def add_page_pop_up_cancel(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_add_page_prompt_view)
        logging.info("At Add Page Pop Up")
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_copy_add_page_cancel)
        assert current_button
        current_button.mouse_click()

    def add_page_pop_up_cancel_yes(self):
        add_page_cancel_yes_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_copy_add_page_cancel_yes)
        assert add_page_cancel_yes_button
        add_page_cancel_yes_button.mouse_click()
    
    def add_page_pop_up_cancel_no(self):
        add_page_cancel_no_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_copy_add_page_cancel_no)
        assert add_page_cancel_no_button
        add_page_cancel_no_button.mouse_click()

    def click_on_copy_preview(self):
        preview_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_preview_large)
        assert preview_button['visible'] == True
        preview_button.mouse_click()
    
    def click_on_copy_preview_add_page_button(self):
        preview_add_page_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_add_page)
        assert preview_add_page_button['visible'] == True
        preview_add_page_button.mouse_click()
    
    def verify_image_preview(self, objectName):
        preview = self.spice.wait_for(objectName)
        assert preview
    
    def duplex_add_page_pop_up_add_more(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.flatbed_two_sided_screen, timeout = 15.0)
        logging.info("At Duplex Add Page Pop Up")
        time.sleep(2)
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_duplex_add_page_add)
        assert current_button
        current_button.mouse_click()
    
    def duplex_add_page_pop_up_finish(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.flatbed_two_sided_screen, timeout = 15.0)
        logging.info("At Duplex Add Page Pop Up")
        time.sleep(2)
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_duplex_add_page_finish, timeout = 15.0)
        assert current_button
        current_button.mouse_click()
    
    def duplex_add_page_pop_up_cancel(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.flatbed_two_sided_screen, timeout = 15.0)
        logging.info("At Duplex Add Page Pop Up")
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_duplex_add_page_cancel, timeout = 15.0)
        assert current_button
        current_button.mouse_click()
    
    def click_on_preview_expand(self):
        expand_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_expand_secondarypanel)
        assert expand_button
        expand_button.mouse_click()
    
    def click_on_preview_collapse(self):
        collapse_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_collapse_secondarypanel)
        assert collapse_button
        collapse_button.mouse_click()
    
    def cancel_copy_preview(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_copyScreen)
        #wait for toast message to be removed
        time.sleep(5)
        home_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_home)
        assert home_button
        home_button.mouse_click()

        assert self.spice.wait_for(CopyAppWorkflowObjectIds.copy_cancel_job_warning_prompt)
        logging.info("At Cancel Job Warning Prompt")
        cancel_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_cancel_job_warning_prompt_primary_button)
        assert cancel_button
        cancel_button.mouse_click()
    
    def click_on_copy_duplex_continue_button(self):
        continue_button = self.spice.wait_for(CopyAppWorkflowObjectIds.duplex_copy_continue)
        assert continue_button
        continue_button.mouse_click()
        
    def goto_stamp_menu(self, select_option = True):
        """
        Navigate to the Stamp menu in the Copy options view.
        """
        logging.info("Navigated to the Stamp menu.")
        self.homemenu.menu_navigation(self.spice, CopyAppWorkflowObjectIds.view_copySettingsView, CopyAppWorkflowObjectIds.list_stamp_menu, select_option = False, scrollbar_objectname = CopyAppWorkflowObjectIds.copy_options_scrollbar)
        time.sleep(2)
        if select_option == True:
            self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_menu).mouse_click() 
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_view)
        else:
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_value)
            
    def verify_stamp_menu_constraint(self):
        """
        Verify the Stamp menu constraint in the Copy options view.
        """
        self.goto_stamp_menu(select_option = False)
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_menu)
        time.sleep(2)
        assert current_button["constrained"] == True
    ...
    def back_from_stamp_location_view(self):
        ...
        backButton = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_location_view_back_button)
    ...
    def verify_stamp_menu_value(self, net, location, setting_values):
        """
        Verify the Stamp menu value in the Copy options view.
        """
        ...
        setting_id = self.stamp_location_dict[location][1]
        ui_setting_string = self.spice.query_item(setting_id + " SpiceText[visible=true]")["text"]
        
        for setting_value in setting_values:
            if setting_value in self.stamp_content_format_string_dict:
                cstring_id = self.stamp_content_format_string_dict[setting_value][0]
            else:
                customString = True
    ...
    def goto_stamp_settings_view(self, location, select_option = True):
        ...
        if location not in self.stamp_location_dict:
            raise ValueError(f"Invalid location: {location}")

        menu_item_id, value_id = self.stamp_location_dict[location]

        self.homemenu.menu_navigation(self.spice, CopyAppWorkflowObjectIds.list_stamp_view, menu_item_id, select_option = False, scrollbar_objectname = CopyAppWorkflowObjectIds.stamp_view_scroll_bar)
        time.sleep(2)
        
        if select_option == True:
            self.spice.wait_for(menu_item_id).mouse_click()
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_settings_view)
        else:
    ...
    def back_from_stamp_setting_view(self):
        ...
        """
        logging.info("Back to the Stamp menu.")
        doneButton = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_settings_done_button)
    ...
    def goto_stamp_content_view(self):
        ...
        self.workflow_common_operations.goto_item(f"{CopyAppWorkflowObjectIds.stamp_settings_content_button}", CopyAppWorkflowObjectIds.list_stamp_settings_view, scrollbar_objectname = CopyAppWorkflowObjectIds.stamp_settings_view_scroll_bar)
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_content_view)
    ...
    def back_from_stamp_contents_view(self):
        ...
        doneButton = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_content_done_button)
        doneButton.mouse_click()
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_settings_view)
    ...
    def select_stamp_content_option(self, option, checked):
        ...
        """
        for i in option:
            row_id, checkBox_id = self.stamp_content_checkbox_dict[i]
            self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_content_view, row_id, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
            ...
            if checked is True:
                if checkBoxProperties["checked"] is False:
                    ...
                    time.sleep(2)
                    assert checkBoxProperties["checked"] is True
            else:
                if checkBoxProperties["checked"] is True:
                    checkBoxElement.mouse_click()
                    time.sleep(2)
                    assert checkBoxProperties["checked"] is False    
    ...
    def verify_stamp_content_exist(self, option, visible=False):
        ...
        """
        for i in option:
            row_id = self.stamp_content_checkbox_dict[i][0]
            if visible == False:
                try:
                    self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_content_view, row_id, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
    ...
    def verify_stamp_content_constraint(self, net, option):
        ...
        row_id, checkBox_id = self.stamp_content_checkbox_dict[option]
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_content_view, row_id, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
        checkBoxElement = self.spice.wait_for(checkBox_id + " MouseArea")
        checkBoxElement.mouse_click()
        
        self.spice.wait_for(CopyAppWorkflowObjectIds.view_constraint_message)
        self.validate_string_id_in_current_view(CopyAppWorkflowObjectIds.view_constraint_message, message_dict.get(option), net)
    ...
    def goto_stamp_edit_order_view(self):
        ...
        editOrderButton = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_content_edit_button)
        editOrderButton.mouse_click()
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_edit_order_view)
    ...
    def back_from_stamp_order_view(self):
        ...
        doneButton = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_edit_order_done_button)
        doneButton.mouse_click()
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_content_view)
    ...
    def edit_stamp_content_order(self, option, direction, num_of_moves=1):
        ...
        """  
        direction_dict = {
            "up": CopyAppWorkflowObjectIds.stamp_edit_order_up_button,
            "down": CopyAppWorkflowObjectIds.stamp_edit_order_done_button
        }
        
        row_id, radio_id = self.stamp_content_radio_dict[option]
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_edit_order_view, row_id, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
    ...
    def goto_stamp_content_text_field(self):
        ...
        self.workflow_common_operations.goto_item(f"{CopyAppWorkflowObjectIds.stamp_settings_content_button}", CopyAppWorkflowObjectIds.list_stamp_settings_view, select_option = False, scrollbar_objectname = CopyAppWorkflowObjectIds.stamp_settings_view_scroll_bar)
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_settings_text_feild)
    ...
    def add_user_defined_stamp_content(self, custom_texts):
        ...
        for custom_text in custom_texts:
            stamp_content_textbox = self.spice.wait_for(f"{CopyAppWorkflowObjectIds.list_stamp_settings_view} {CopyAppWorkflowObjectIds.stamp_settings_text_feild}")
            stamp_content_textbox.mouse_click()
            stamp_content_textbox.__setitem__('text', custom_text)
            time.sleep(2)
            
            keyword_ok = self.spice.wait_for(CopyAppWorkflowObjectIds.button_keyboard_ok)
    ...
    def delete_stamp_content_on_text_field(self, index):
        ...
        """
        self.goto_stamp_content_text_field()
        stamp_content_button = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_settings_bubbule + str(index) + " #bubbleMouseArea")
        stamp_content_button.mouse_click()
        time.sleep(2)
        
        stamp_content_delete_button = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_settings_bubbule + str(index) + " #clearButton")
    ...
    def verify_stamp_content_text_on_text_field(self, net, index, bubbleText):
        ...
        customString = False
        stamp_content_button_text = self.spice.query_item(CopyAppWorkflowObjectIds.stamp_settings_bubbule + str(index) + " #EmailText")["text"]
        
        if bubbleText in self.stamp_content_format_string_dict:
            cstring_id = self.stamp_content_format_string_dict[bubbleText][0]
        else:
            customString = True
    ...
    def verify_stamp_user_defined_constraint_message(self, net):
        ...
        """
        self.spice.wait_for(CopyAppWorkflowObjectIds.view_constraint_message)
        self.validate_string_id_in_current_view(CopyAppWorkflowObjectIds.view_constraint_message, 'cUserCustomStampsLimited', net)
    ...
    def goto_stamp_starting_page_spinBox(self):
        ...
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_starting_page_spinBox, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_starting_page_spinBox)
    ...
    def change_stamp_starting_page(self, value=1):
        ...
        startingPageElement = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_starting_page_spinBox)
    ...
    def verify_stamp_starting_page_option_visible(self, visible=True):
        ...
        if visible == True:
            ...
            try:
                self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_starting_page_spinBox, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
                startingPageElement = self.spice.wait_for(CopyAppWorkflowObjectIds.row_stamp_starting_page_spinBox, timeout=5.0)
    ...
    def goto_stamp_starting_number_spinBox(self):
        ...
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_starting_number_spinBox, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_starting_number_spinBox)
    ...
    def change_stamp_starting_number(self, value=1):
        ...
        startingNumberElement = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_starting_number_spinBox)
    ...
    def verify_stamp_starting_number_option_visible(self, visible=True):
        ...
        if visible == True:
            ...
            try:
                self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_starting_number_spinBox, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
                startingNumberElement = self.spice.wait_for(CopyAppWorkflowObjectIds.row_stamp_starting_number_spinBox, timeout=5.0)
    ...
    def goto_stamp_number_of_digit_spinBox(self):
        ...
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_number_of_digit_spinBox, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_number_of_digit_spinBox)
    ...
    def change_stamp_number_of_digit(self, value=1):
        ...
        numberOfDigitElement = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_number_of_digit_spinBox)
    ...
    def goto_stamp_page_numbering_option(self, select_option = True):
        ...
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_page_numbering_comboBox, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
        time.sleep(2)
        
        if select_option == True:
            self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_page_numbering_comboBox).mouse_click()    
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_stamp_comboBox, timeout = 9.0)
        else:
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_page_numbering_comboBox)
    ...
    def select_stamp_page_numbering_option(self, option):
        ...
        page_numbering_dict = {
            "number": f"{CopyAppWorkflowObjectIds.combo_stamp_number}",
            "pagePlusNumber": f"{CopyAppWorkflowObjectIds.combo_stamp_page_plus_number}",
            "hyphenNumber": f"{CopyAppWorkflowObjectIds.combo_stamp_hyphen_number}"
        }
        
        to_select_item = page_numbering_dict.get(option)
        self.workflow_common_operations.goto_item(to_select_item, CopyAppWorkflowObjectIds.view_stamp_comboBox, scrollbar_objectname = CopyAppWorkflowObjectIds.stamp_comboBox_scrollbar)
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_settings_view, timeout = 9.0)
    ...
    def verify_stamp_page_numbering_option(self, net, setting_value):
        ...
        setting_id = CopyAppWorkflowObjectIds.stamp_page_numbering_comboBox
        cstring_id = self.stamp_page_numbering_format_string_dict[setting_value.lower()][0]
    ...
    def goto_stamp_text_color_option(self, select_option = True):
        ...
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_text_color, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
        time.sleep(2)
        
        if select_option == True:
            self.spice.wait_for(CopyAppWorkflowObjectIds.row_stamp_text_color).mouse_click() 
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_text_color)
        else:
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.row_stamp_text_color_value)
    ...
    def select_stamp_text_color_option(self, option):
        ...
        stamp_text_color_dict = {
            "black": CopyAppWorkflowObjectIds.row_text_color_black,
            "yellow": CopyAppWorkflowObjectIds.row_text_color_yellow,
            "green": CopyAppWorkflowObjectIds.row_text_color_green,
            "red": CopyAppWorkflowObjectIds.row_text_color_red,
            "blue": CopyAppWorkflowObjectIds.row_text_color_blue,
            "skyBlue": CopyAppWorkflowObjectIds.row_text_color_skyBlue,
            "purple": CopyAppWorkflowObjectIds.row_text_color_purple
        }

        to_select_item = stamp_text_color_dict.get(option)
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_view_text_color, to_select_item, top_item_id='#SpiceHeaderVar2', scroll_step=1)
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_stamp_settings_view, timeout = 9.0)
    ...
    def verify_stamp_text_color_option(self, net, setting_value):
        ...
        setting_id = CopyAppWorkflowObjectIds.row_stamp_text_color_value
        cstring_id = self.stamp_text_color_format_string_dict[setting_value.lower()][0]
    ...
    def goto_stamp_text_font_option(self, select_option = True):
        ...
        """
        logging.info("Navigated to the Stamp page numbering option.")
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_text_font_comboBox, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
        time.sleep(2)
        
        if select_option == True:
            self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_text_font_comboBox).mouse_click()    
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_stamp_comboBox, timeout = 9.0)
        else:
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_text_font_comboBox)
    ...
    def select_stamp_text_font_option(self, option):
        ...
        text_font_dict = {
            "antique": f"{CopyAppWorkflowObjectIds.combo_stamp_antiqueOlive}",
            "century": f"{CopyAppWorkflowObjectIds.combo_stamp_centurySchoolbook}",
            "garamond": f"{CopyAppWorkflowObjectIds.combo_stamp_garamond}",
            "letter": f"{CopyAppWorkflowObjectIds.combo_stamp_letterGothic}",
        }
    ...
    def verify_stamp_text_font_option(self, net, setting_value):
        ...
        setting_id = CopyAppWorkflowObjectIds.stamp_text_font_comboBox
        cstring_id = self.stamp_text_font_format_string_dict[setting_value.lower()][0]
    ...
    def goto_stamp_text_size_option(self, select_option = True):
        ...
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_text_size_comboBox, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
        time.sleep(2)
        
        if select_option == True:
            self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_text_size_comboBox).mouse_click()    
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_stamp_comboBox, timeout = 9.0)
        else:
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_text_size_comboBox)
    ...
    def select_stamp_text_size_option(self, option):
        ...
        text_size_dict = {
            "8point": f"{CopyAppWorkflowObjectIds.combo_stamp_eightPoint}",
            "12point": f"{CopyAppWorkflowObjectIds.combo_stamp_twelvePoint}",
            "20point": f"{CopyAppWorkflowObjectIds.combo_stamp_twentyPoint}",
        }
    ...
    def verify_stamp_text_size_option(self, net, setting_value):
        ...
        setting_id = CopyAppWorkflowObjectIds.stamp_text_size_comboBox
        cstring_id = self.stamp_text_size_format_string_dict[setting_value.lower()][0]
    ...
    def goto_stamp_white_background_option(self):
        ...
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_stamp_settings_view, CopyAppWorkflowObjectIds.row_stamp_white_background_checkBox, top_item_id='#SpiceHeaderVar2', select_option=False, scroll_step=1)
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_white_background_checkBox)
    ...
    def set_stamp_white_background_option(self, checked):
        ...
        white_background_checkBox = self.spice.wait_for(CopyAppWorkflowObjectIds.stamp_white_background_checkBox)
        if checked is True:
    ...
    def goto_copy_option_staple(self):
        ...
        self.homemenu.menu_navigation(self.spice,CopyAppWorkflowObjectIds.view_copySettingsView, CopyAppWorkflowObjectIds.list_copy_staple, scrollbar_objectname = CopyAppWorkflowObjectIds.copy_options_scrollbar)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_copy_staple_value)    
    ...
    def goto_copy_option_punch(self):
        ...
        self.homemenu.menu_navigation(self.spice,CopyAppWorkflowObjectIds.view_copySettingsView, CopyAppWorkflowObjectIds.list_copy_punch, scrollbar_objectname = CopyAppWorkflowObjectIds.copy_options_scrollbar)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_copy_punch_value)

    def select_staple_option(self, option):

        self.goto_copy_option_staple()

        staple_dict = {
            "leftTwoPoints": {
                "item_id": CopyAppWorkflowObjectIds.radio_staple_leftTwoPoints,
                "row_id": CopyAppWorkflowObjectIds.row_staple_leftTwoPoints
                },
            "topLeftOnePointAny": {
                "item_id": CopyAppWorkflowObjectIds.radio_staple_topLeftOnePointAny,
                "row_id": CopyAppWorkflowObjectIds.row_staple_topLeftOnePointAny
                },    
        }

        to_select_item = staple_dict.get(option)
        menu_item_id = to_select_item
        self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.copy_stapleMenuSelectionList,menu_item_id=to_select_item["row_id"],top_item_id='#SpiceHeaderVar2',select_option = True, scroll_step = 8)
        time.sleep(10)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_copySettingsView, timeout = 9.0)

    def select_punch_option(self, option):

        self.goto_copy_option_punch()

        punch_dict = {
            "leftTwoPointDin": {
                "item_id": CopyAppWorkflowObjectIds.radio_punch_leftTwoPointDin,
                "row_id": CopyAppWorkflowObjectIds.row_punch_leftTwoPointDin
                },
            "rightTwoPointDin": {
                "item_id": CopyAppWorkflowObjectIds.radio_punch_rightTwoPointDin,
                "row_id": CopyAppWorkflowObjectIds.row_punch_rightTwoPointDin
                },    
        }

        to_select_item = punch_dict.get(option)
        menu_item_id = to_select_item
        self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.copy_punchMenuSelectionList,menu_item_id=to_select_item["row_id"],top_item_id='#SpiceHeaderVar2',select_option = True, scroll_step = 8)
        time.sleep(10)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_copySettingsView, timeout = 9.0)        

    def tray_empty_load_paperscreen(self,option):

        assert self.spice.wait_for(CopyAppWorkflowObjectIds.copy_load_paper_alert, timeout = 20)
        load_paper_option = {
            "ok" : CopyAppWorkflowObjectIds.copy_load_paper_ok,
            "Hide": CopyAppWorkflowObjectIds.copy_load_paper_hide,
            "cancel": CopyAppWorkflowObjectIds.copy_cancel_button
        }
        to_click_button= load_paper_option.get(option)
        self.spice.wait_for(to_click_button)
        current_button= self.spice.wait_for(to_click_button)
        current_button.mouse_click()
    
    def get_number_of_footer_spinBox_copies(self):
        numCopiesElement = self.spice.wait_for(CopyAppWorkflowObjectIds.quickCopyFooterSpinBox)
        value = numCopiesElement.__getitem__('value')
        return value

    def perform_copy_from_homescreen_footer(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.homeScreenFooter+" "+CopyAppWorkflowObjectIds.quickCopyFooter)
        copyButton=self.spice.wait_for(CopyAppWorkflowObjectIds.homeScreenFooterCopyButton)
        copyButton.mouse_click()
        
    def check_footer_copy_start_button_disabled(self):
        """
        Check Copy start button disabled while copy job in progress
        """
        logging.info("Checking copy start button is disabled")
        copyButton=self.spice.wait_for(CopyAppWorkflowObjectIds.homeScreenFooterCopyButton)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.homeScreenFooterCopyButton)["enabled"] is False, 'Button is enabled'

    def set_number_of_footer_spinBox_copies(self, value):
        numCopiesElement = self.spice.wait_for(CopyAppWorkflowObjectIds.quickCopyFooterSpinBox)
        numCopiesElement.__setitem__('value', value)

    def get_quickCopy_footer(self):
        return self.spice.wait_for(CopyAppWorkflowObjectIds.homeScreenFooter+" "+CopyAppWorkflowObjectIds.quickCopyFooter)

    def validate_string_id_on_main_action_button(self, cdm, expected_string_id):
        # Get translated button string id in device language
        expected_copy_text_button = CommonActions.get_translated_text_in_device_language(cdm, expected_string_id)
    ...
    def validate_copy_and_scan_in_progress(self, scan_progress = True):
        self.spice.wait_for(CopyAppWorkflowObjectIds.view_copyScreen, timeout =9.0)
        cancel_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_cancel_button)
        assert cancel_button['visible'] == True
        time.sleep(2)
        copy_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_button)
        if scan_progress:
            assert copy_button['enabled'] == False
        else:
            assert copy_button['enabled'] == True
    ...
    def click_home_button(self) -> bool:
        """
        Clicks the Home button in the top left corner of the UI from the Copy app

        Args:
            No arguments
        
        Returns:
            bool: True if the 'click' was performed and False otherwise
        
        Raises:
            TimeoutError: If the Home button is not visible after 'wait_until' default timeout time
        """
        copy_home_button = self.workflow_common_operations.get_element(CopyAppWorkflowObjectIds.button_home)
        self.spice.wait_until(lambda: copy_home_button.is_visible())
        if not copy_home_button:
            logging.error(f"Failed to get Copy Home Button: {CopyAppWorkflowObjectIds.button_home}")
            return False
        return self.workflow_common_operations.click(copy_home_button)
    ...
    def click_on_main_action_button_in_main_panel(self):
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_startCopy_mainpanel)
        current_button.mouse_click()

    def start_copy_after_preview(self):
        # For large screen preview can be verified up front So click on start copy button on detail panel
        current_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_button, timeout=30)
        self.spice.wait_until(lambda: current_button['enabled'] == True, 10.0)
        current_button.mouse_click()
    ...
    def click_on_cancel_job_warning_prompt(self, button):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.copy_cancel_job_warning_prompt)
        click_Button = self.spice.wait_for(button)
        click_Button.mouse_click()

    def click_on_finish_in_add_page_prompt(self):
        finish_button = self.spice.wait_for("#finish")
        finish_button.mouse_click()

    def click_on_inactivity_timeout_prompt_continue_button(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.copy_cancel_job_warning_prompt, timeout=15.0), "Not at Inactivity Timeout Prompt"
        logging.info("At Inactivity Timeout Prompt")
        continue_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_cancel_job_warning_prompt_primary_button)
        continue_button.mouse_click()
    
    def click_on_inactivity_timeout_prompt_cancel_button(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.copy_cancel_job_warning_prompt, timeout=15.0), "Not at Inactivity Timeout Prompt"
        logging.info("At Inactivity Timeout Prompt")
        cancel_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_cancel_job_warning_prompt_secondary_button)
        cancel_button.mouse_click()
    ...
    def goto_copy_option_fold(self):
        ...
        """
        logging.info("Go to punch option menu")
        self.homemenu.menu_navigation(self.spice,CopyAppWorkflowObjectIds.view_copySettingsView, CopyAppWorkflowObjectIds.list_copy_fold, scrollbar_objectname = CopyAppWorkflowObjectIds.copy_options_scrollbar)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_copy_fold_value)

    def select_fold_option(self, option):

        self.goto_copy_option_fold()

        fold_dict = {
            "V-fold": {
                "item_id": CopyAppWorkflowObjectIds.radio_fold_Vfold,
                "row_id": CopyAppWorkflowObjectIds.row_fold_Vfold
                },
            "C-fold": {
                "item_id": CopyAppWorkflowObjectIds.radio_fold_Cfold,
                "row_id": CopyAppWorkflowObjectIds.row_fold_Cfold
                },    
        }

        to_select_item = fold_dict.get(option)
        menu_item_id = to_select_item
        self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.copy_foldMenuSelectionList,menu_item_id=to_select_item["row_id"],top_item_id='#SpiceHeaderVar2',select_option = True, scroll_step = 8)
        time.sleep(5)

        btn = self.spice.wait_for("#copyFoldView #BackButton", timeout=9.0)
        time.sleep(2)
        btn.mouse_click()
        time.sleep(2)
    ...
    def verify_copy_booklet_format_selection_option(self, net, setting, setting_value):
        """
        This method compares the booklet format setting string with the expected string from string id
        Args:
            UI should be in Copy booklet format settings view
            setting: booklet format to be validated -> e.g.: "booklet_format", "on or off".
            setting_value: Value of the setting
        """
        expected_string_id = ""
        actual_string_id = ""
        self.homemenu.menu_navigation(self.spice,CopyAppWorkflowObjectIds.view_copySettingsView, 
                                      CopyAppWorkflowObjectIds.list_copySettings_booklet, 
                                      scrollbar_objectname = CopyAppWorkflowObjectIds.copy_options_scrollbar, select_option=False)
        if setting == "booklet_format":
            expected_string_id = self.booklet_format_dict[setting_value.lower()][0]
            actual_string_id = CopyAppWorkflowObjectIds.list_copySettings_booklet_value
    ...
    def goto_copy_booklet_option(self):
        ...
        self.homemenu.menu_navigation(self.spice,CopyAppWorkflowObjectIds.view_copySettingsView, 
                                      CopyAppWorkflowObjectIds.list_copySettings_booklet, 
                                      scrollbar_objectname = CopyAppWorkflowObjectIds.copy_options_scrollbar)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_copySettings_booklet)
        time.sleep(3)
    ...
    def select_booklet_option(self, option):
        ...
        @return:
        """
        self.goto_copy_booklet_option()
        if (option == 'bookletFormat'):
            booklet_button = self.spice.wait_for(CopyAppWorkflowObjectIds.row_booklet_format_option)  
        elif(option == 'bordersOnEachPage'):
            booklet_button = self.spice.wait_for(CopyAppWorkflowObjectIds.row_borders_on_each_page_option)
        elif(option == 'foldAndStitch'):
            booklet_button = self.spice.wait_for(CopyAppWorkflowObjectIds.row_booklet_fold_and_stitch_option)
        else:
            booklet_button = self.spice.wait_for(CopyAppWorkflowObjectIds.row_booklet_fold_and_stitch_custom_option)
        ...
        back_button = self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_copySettings_booklet +" "+ CopyAppWorkflowObjectIds.button_back)
        back_button.mouse_click()

    def verify_copy_booklet_borders_on_each_page_constrained(self, constrained: bool = True):
        """
        Go to booklet option menu and verify that option is cosntrained
        @return:
        """
        self.goto_copy_booklet_option()
        borders_on_each_page_button = self.spice.wait_for(CopyAppWorkflowObjectIds.row_borders_on_each_page_option)
        if constrained == False:
            assert borders_on_each_page_button["constrained"] == False
        else:
            assert borders_on_each_page_button["constrained"] == True
            assert borders_on_each_page_button["checked"] == False
            borders_on_each_page_button = self.spice.wait_for(CopyAppWorkflowObjectIds.row_borders_on_each_page_option + " SpiceText")
            borders_on_each_page_button.mouse_click()
            time.sleep(3)
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_constraint_message)
    ...
    def verify_copy_fold_and_stitch_custom_constrained(self, constrained: bool = True):
        """
        Go to booklet option menu and verify that option is cosntrained
        @return:
        """
        self.goto_copy_booklet_option()
        fold_and_stitch_custom_button = self.spice.wait_for(CopyAppWorkflowObjectIds.row_booklet_fold_and_stitch_custom_option)
        if constrained == False:
            assert fold_and_stitch_custom_button["constrained"] == False
        else:
            assert fold_and_stitch_custom_button["constrained"] == True
            assert fold_and_stitch_custom_button["checked"] == False
            fold_and_stitch_custom_button = self.spice.wait_for(CopyAppWorkflowObjectIds.row_booklet_fold_and_stitch_custom_option + " SpiceText")
            fold_and_stitch_custom_button.mouse_click()
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_constraint_message)
    ...
    def verify_copy_booklet_constrained(self, net, constrained_message : str = ""):
        """
        Go to booklet option menu and verify that option is cosntrained
        @return:
        """
    ...
    def goto_copy_watermark_option(self):
        """
        Go to watermark option menu
        @return:
        """
        logging.info("Go to watermark option menu")
        self.homemenu.menu_navigation(self.spice,CopyAppWorkflowObjectIds.view_copySettingsView, 
                                      CopyAppWorkflowObjectIds.list_watermark, 
                                      scrollbar_objectname = CopyAppWorkflowObjectIds.copy_options_scrollbar)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_watermark)
        time.sleep(3)
    ...
    def verify_watermark_option_list_screen(self, option):
        ...
        :return:
        """
        watermark_type_view = self.spice.wait_for(CopyAppWorkflowObjectIds.combo_watermarkType)
        ...
        watermark_text_option = self.spice.wait_for(CopyAppWorkflowObjectIds.list_watermark_text)
        self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.list_view_watermark, menu_item_id=CopyAppWorkflowObjectIds.list_watermark_text, select_option=False)
        ...
        first_page_only = self.spice.wait_for(CopyAppWorkflowObjectIds.list_watermark_first_page_only)
        self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.list_view_watermark, menu_item_id=CopyAppWorkflowObjectIds.list_watermark_first_page_only, select_option=False)
    ...
    def goto_watermark_type_screen(self):
        ...
        """
        time.sleep(3)
        self.workflow_common_operations.scroll_to_position_vertical(0, CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar)
        menu_item_id = [CopyAppWorkflowObjectIds.row_combo_watermarkType, CopyAppWorkflowObjectIds.combo_watermarkType]
        self.workflow_common_operations.goto_item(menu_item_id, CopyAppWorkflowObjectIds.list_view_watermark, scrollbar_objectname = CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_watermarkType)

    def select_copy_watermark_type(self, option):
        """
        ...
        watermark_options_dict = {
            "none": f"{CopyAppWorkflowObjectIds.combo_watermarkType_option_none}",
            "text": f"{CopyAppWorkflowObjectIds.combo_watermarkType_option_text}"
        }
        ...
        current_button.mouse_click()
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_watermark, timeout = 9.0)

    def verify_copy_watermark_constrained(self, net, constrained_message : str = ""):
        """
        Go to watermark menu and verify that option is cosntrained
        @return:
        """
        constrained_message = CopyAppWorkflowObjectIds.scan_settings_watermark_constraints_message
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_constraint_message)
    ...
    def goto_copy_watermark_text(self):
        '''
        UI should be in Copy watermark screen.
        Navigates to watermark text screen starting from watermark screen.
        '''
        self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_watermark)
        watermark_text_view = self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_watermark + " " + CopyAppWorkflowObjectIds.button_watermark_text_MoreOptions, timeout = 10)
        ...
        if (watermark_text_view["enabled"] == True and watermark_text_view["visible"] == True):
            ...
            self.spice.wait_for(CopyAppWorkflowObjectIds.view_watermark_text_MoreOptions_view)
        time.sleep(3)

    def select_copy_watermark_text(self, option):

        self.goto_copy_watermark_text()

        watermark_text_dict = {
            "draft": {
                "item_id": CopyAppWorkflowObjectIds.radio_watermark_text_draft,
                "row_id": CopyAppWorkflowObjectIds.row_watermark_text_draft
                },
            "confidential": {
                "item_id": CopyAppWorkflowObjectIds.radio_watermark_text_confidential,
                "row_id": CopyAppWorkflowObjectIds.row_watermark_text_confidential
                },
            "secret": {
                "item_id": CopyAppWorkflowObjectIds.radio_watermark_text_secret,
                "row_id": CopyAppWorkflowObjectIds.row_watermark_text_secret
                },
            "topSecret": {
                "item_id": CopyAppWorkflowObjectIds.radio_watermark_text_top_secret,
                "row_id": CopyAppWorkflowObjectIds.row_watermark_text_top_secret
                },
            "urgent": { 
                "item_id": CopyAppWorkflowObjectIds.radio_watermark_text_urgent,
                "row_id": CopyAppWorkflowObjectIds.row_watermark_text_urgent
            }
        }

        to_select_item = watermark_text_dict.get(option)
        self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.view_watermark_text_MoreOptions_view, menu_item_id=to_select_item["row_id"])
        time.sleep(10)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_watermark, timeout = 9.0)
    ...
    def enter_new_custom_watermark_text(self, custom_text):
        ...
        '''
        self.workflow_common_operations.scroll_vertical_row_item_into_view(CopyAppWorkflowObjectIds.list_view_watermark, CopyAppWorkflowObjectIds.watermark_text_field, select_option=False)
        file_name_textbox = self.spice.wait_for(f"{CopyAppWorkflowObjectIds.list_view_watermark} {CopyAppWorkflowObjectIds.watermark_text_field_text}")
        file_name_textbox.mouse_click()
        file_name_textbox.__setitem__('displayText', custom_text)
        keyword_ok = self.spice.wait_for(CopyAppWorkflowObjectIds.button_keyboard_ok)
        keyword_ok.mouse_click()
   
    def verify_copy_watermark_selection_option(self, net, setting_value):
        """
        ...
        actual_string_id = CopyAppWorkflowObjectIds.list_watermark_value
    ...
    def select_first_page_only_option(self):
        ...
        @return:
        """
        menu_item_id = [CopyAppWorkflowObjectIds.row_combo_text_font, CopyAppWorkflowObjectIds.combo_text_font]
        self.workflow_common_operations.goto_item(menu_item_id, CopyAppWorkflowObjectIds.list_view_watermark, scrollbar_objectname = CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar,
                                                  select_option=False, scrolling_value=0.07)
        first_page_only_checkbox = self.spice.wait_for(CopyAppWorkflowObjectIds.checkbox_first_page_only)
    ...
    def goto_watermark_text_font_screen(self):
        ...
        menu_item_id = [CopyAppWorkflowObjectIds.row_combo_text_font, CopyAppWorkflowObjectIds.combo_text_font]
        self.workflow_common_operations.goto_item(menu_item_id, CopyAppWorkflowObjectIds.list_view_watermark, scrollbar_objectname = CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar,
                                                  scrolling_value=0.088)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_text_font)

    def select_watermark_text_font(self, option):
        """
        ...
        text_font_options_dict = {
            "Letter Gothic": f"{CopyAppWorkflowObjectIds.combo_text_font_letter_gothic}",
            "Antique Olive": f"{CopyAppWorkflowObjectIds.combo_text_font_antique_olive}",
            "Century Schoolbook": f"{CopyAppWorkflowObjectIds.combo_text_font_century_schoolbook}",
            "Garamond Antiqua": f"{CopyAppWorkflowObjectIds.combo_text_font_garamond}"
        }
        to_select_item = text_font_options_dict.get(option)
        time.sleep(2)
        self.workflow_common_operations.goto_item(to_select_item, CopyAppWorkflowObjectIds.view_text_font, 
            scrollbar_objectname = CopyAppWorkflowObjectIds.copy_combobox_scrollbar, select_option=False)
        current_button = self.spice.query_item(to_select_item + "")
        current_button.mouse_click()
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_watermark, timeout = 9.0)
    ...
    def goto_watermark_text_size_screen(self):
        ...
        time.sleep(3)
        self.workflow_common_operations.scroll_to_position_vertical(0, CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar)
        menu_item_id = [CopyAppWorkflowObjectIds.row_combo_text_size, CopyAppWorkflowObjectIds.combo_text_size]
        self.workflow_common_operations.goto_item(menu_item_id, CopyAppWorkflowObjectIds.list_view_watermark,
                                                  scrollbar_objectname = CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar,
                                                  scrolling_value=0.09)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.view_text_size)

    def select_watermark_text_size(self, option):
        """
        ...
        text_size_options_dict = {
            "thirtyPoint": f"{CopyAppWorkflowObjectIds.combo_text_size_thirtyPoint}",
            "fortyPoint": f"{CopyAppWorkflowObjectIds.combo_text_size_fortyPoint}",
            "sixtyPoint": f"{CopyAppWorkflowObjectIds.combo_text_size_sixtyPoint}"
        }
        to_select_item = text_size_options_dict.get(option)
        time.sleep(2)
        self.workflow_common_operations.goto_item(to_select_item, CopyAppWorkflowObjectIds.view_text_size, 
            scrollbar_objectname = CopyAppWorkflowObjectIds.standard_sizes_scrollbar, select_option=False)
        current_button = self.spice.query_item(to_select_item + "")
        current_button.mouse_click()
        time.sleep(2)
    ...
    def goto_watermark_text_color_screen(self):
        ...
        self.workflow_common_operations.scroll_to_position_vertical(0, CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar)
        self.workflow_common_operations.goto_item(CopyAppWorkflowObjectIds.list_text_color,
                                                  CopyAppWorkflowObjectIds.list_view_watermark,
                                                  scrollbar_objectname = CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar,
                                                  scrolling_value=0.095)
        time.sleep(2)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_text_color)

    def select_watermark_text_color(self, option):

        self.goto_watermark_text_color_screen()

        watermark_text_color_dict = {
            "black": {
                "item_id": CopyAppWorkflowObjectIds.radio_text_color_black,
                "row_id": CopyAppWorkflowObjectIds.row_text_color_black
                },
            "yellow": {
                "item_id": CopyAppWorkflowObjectIds.radio_text_color_yellow,
                "row_id": CopyAppWorkflowObjectIds.row_text_color_yellow
                },
            "green": {
                "item_id": CopyAppWorkflowObjectIds.radio_text_color_green,
                "row_id": CopyAppWorkflowObjectIds.row_text_color_green
                },
            "red": {
                "item_id": CopyAppWorkflowObjectIds.radio_text_color_red,
                "row_id": CopyAppWorkflowObjectIds.row_text_color_red
                },
            "blue": {
                "item_id": CopyAppWorkflowObjectIds.radio_text_color_blue,
                "row_id": CopyAppWorkflowObjectIds.row_text_color_blue
            },
            "skyBlue": {
                "item_id": CopyAppWorkflowObjectIds.radio_text_color_skyBlue,
                "row_id": CopyAppWorkflowObjectIds.row_text_color_skyBlue
            },
            "purple": { 
                "item_id": CopyAppWorkflowObjectIds.radio_text_color_purple,
                "row_id": CopyAppWorkflowObjectIds.row_text_color_purple
            }
        }

        to_select_item = watermark_text_color_dict.get(option)
        self.workflow_common_operations.scroll_vertical_row_item_into_view(screen_id=CopyAppWorkflowObjectIds.list_view_text_color, menu_item_id=to_select_item["row_id"])
        time.sleep(10)
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_watermark, timeout = 9.0)
    ...
    def goto_watermark_darkness_option(self):
        ...
        self.workflow_common_operations.scroll_to_position_vertical(0, CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar)
        menu_item_id = [CopyAppWorkflowObjectIds.row_slider_watermark_darkness, CopyAppWorkflowObjectIds.slider_watermark_darkness]
        self.workflow_common_operations.goto_item(menu_item_id, CopyAppWorkflowObjectIds.list_view_watermark, 
                                                  select_option = False, scrolling_value=0.095,
                                                  scrollbar_objectname = CopyAppWorkflowObjectIds.copy_watermark_options_scrollbar)
    ...
    def select_watermark_darkness(self, value: int):
        ...
        """
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.list_view_watermark), "Error: Not in Watermark settings view"
        self.goto_watermark_darkness_option()
        current_slider_value = self.spice.query_item(CopyAppWorkflowObjectIds.slider_watermark_darkness)
        logging.info("Current watermark darkness value is: %s" % current_slider_value["value"])

        watermark_darkness_slider = self.spice.wait_for(CopyAppWorkflowObjectIds.slider_watermark_darkness)
        watermark_darkness_slider.__setitem__('value', value)

        current_slider_value = self.spice.query_item(CopyAppWorkflowObjectIds.slider_watermark_darkness)["value"]
        logging.info("After setting watermark darkness value is: %s" % current_slider_value)
    
    def go_back_to_setting_from_watermark(self):
        self.workflow_common_operations.back_or_close_button_press(f"{CopyAppWorkflowObjectIds.watermark_done_button}", CopyAppWorkflowObjectIds.view_copySettingsView)
      

    def verify_copy_staple_constrained(self):
        """
        Go to booklet option menu and verify that staple option is constrained
        @return:
        """
    ...
    def verify_copy_punch_constrained(self):
        """
        Go to booklet option menu and verify that punch option is cosntrained
        @return:
        """
    ...
    def verify_copy_fold_constrained(self):
        """
        Go to booklet option menu and verify that fold option is cosntrained
        @return:
        """
    ...
    def select_book_mode_options(self, bookmode: str):
        ...
        if bookmode == "scanBothSides":
            book_mode_scan_both_sides_button = self.spice.wait_for(CopyAppWorkflowObjectIds.scan_both_sides_for_book_mode)
            book_mode_scan_both_sides_button.mouse_click()
        elif (bookmode == "skipLeftPage" or bookmode == "skipTopPage"):
            book_mode_skip_left_page_button = self.spice.wait_for(CopyAppWorkflowObjectIds.scan_skip_left_side_for_book_mode)
            book_mode_skip_left_page_button.mouse_click()
        else:
            book_mode_skip_right_page_button = self.spice.wait_for(CopyAppWorkflowObjectIds.scan_skip_right_side_for_book_mode)
            book_mode_skip_right_page_button.mouse_click()    

    def validate_book_mode_instructions_view(self):
        assert self.spice.wait_for(CopyAppWorkflowObjectIds.book_mode_instructions_view)
        
    def book_mode_instructions_page_scan(self):
        self.validate_book_mode_instructions_view()    
        scan_book_mode_instructions_page_scan_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_scan_book_mode_instructions_scan)
        scan_book_mode_instructions_page_scan_button.mouse_click()

    def book_mode_instructions_page_finish(self):
        self.validate_book_mode_instructions_view() 
        scan_book_mode_instructions_page_scan_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_scan_book_mode_instructions_finish)
        scan_book_mode_instructions_page_scan_button.mouse_click()
    
    def book_mode_instructions_page_cancel(self):
        scan_book_mode_instructions_page_cancel_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_scan_book_mode_instructions_cancel)
        scan_book_mode_instructions_page_cancel_button.mouse_click()
        scan_book_mode_instructions_page_cancel_page_yes_button = self.spice.wait_for(CopyAppWorkflowObjectIds.button_scan_add_page_flatbed_duplex_cancel_yes)
        scan_book_mode_instructions_page_cancel_page_yes_button.mouse_click()

    def back_from_scan_mode_option(self):
        back_button = self.spice.query_item(CopyAppWorkflowObjectIds.scan_mode_back_button)
        back_button.mouse_click()

    def handle_two_sided_id_prompt(self, net, id_card_prompt_side= "", button_to_click= ""):
        two_sided_id_prompt = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_2_sided_id_prompt)
        id_card_prompt_text = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_2_sided_id_prompt_text)["text"]
        if id_card_prompt_side == "front":
            assert id_card_prompt_text == self.workflow_common_operations.get_expected_translation_str_by_str_id(net, 'cScanTheFrontSide')
        else:
            assert id_card_prompt_text == self.workflow_common_operations.get_expected_translation_str_by_str_id(net, 'cPrintTheIDCard')

        if button_to_click == "scan":
            scan_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_2_sided_id_scan_button)
            scan_button.mouse_click()
        elif button_to_click == "cancel":
            cancel_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_2_sided_id_cancel_button)
            cancel_button.mouse_click()
        else:
            done_button = self.spice.wait_for(CopyAppWorkflowObjectIds.copy_2_sided_id_done_button)
    ...
    def verify_constrained_message(self, net , constrained_message : str = ""):
        ...
        if(constrained_message != ""):
            constrained_message_text = self.spice.wait_for(CopyAppWorkflowObjectIds.constraint_message_text)["text"]
    ...
    def done_button_present(self,spice, timeout: float = WAIT_TIMEOUT):
        ...
        spice.copy_app.wait_locator_enabled(CopyAppWorkflowObjectIds.done_button, timeout)
        done_button = self.spice.query_item(CopyAppWorkflowObjectIds.done_button)
        spice.wait_until_constrained(done_button,False)

    def press_done_button(self,spice):
        # Press Button "Done"
        spice.copy_app.goto(CopyAppWorkflowObjectIds.done_button)

    def check_print_path_blocked(self, spice):

        # If UI type is workflow Alert App will be opened
        if self.spice.uitype == "Workflow2" :
            spice.wait_for(CopyAppWorkflowObjectIds.alert_app_landing_view)
            alertClose = spice.query_item(CopyAppWorkflowObjectIds.alert_app_landing_view_close_button)
            alertClose.mouse_click()
        
        else:
            spice.statusCenter_dashboard_collapse()
    
    def is_on_copy_screen(self, wait_timeout:float = WAIT_TIMEOUT) -> bool:
        copy_screen = self.spice.wait_for(CopyAppWorkflowObjectIds.view_copyScreen, timeout=wait_timeout)
        if not copy_screen:
            logging.error(f"Failed to get Copy Screen: {CopyAppWorkflowObjectIds.view_copyScreen}")
            return False
        self.spice.wait_until(lambda: copy_screen.is_visible() == True, timeout=wait_timeout)
        return True

    def check_for_cancel_model(self):

        try:
            assert self.spice.wait_for(CopyAppWorkflowObjectIds.copy_active_job_modal)
            okButton = self.spice.wait_for(CopyAppWorkflowObjectIds.ok_button)
            okButton.mouse_click()

        except:
            logging.info("Cancel model not found")    
    
    def validate_constrained_message_on_settings_option(self,net ,option_name, constraints_unavilable = False ):
        ui_string = self.spice.wait_for(CopyAppWorkflowObjectIds.view_constraint_message)["message"]
        option_name = str(LocalizationHelper.get_string_translation(net, option_name))
        if constraints_unavilable:
            expected_string = self.spice.common_operations.get_expected_translation_str_by_str_id(net, 'cThisOptionUnavailable')
        else:
            expected_string = self.spice.common_operations.get_expected_translation_str_by_str_id(net, ['cMessageNotAllowModifySettings', option_name])
        button = self.spice.wait_for(CopyAppWorkflowObjectIds.ok_button)
        button.mouse_click()
        assert ui_string == expected_string, "String mismatch"

    def goto_paper_source(self):
        self.spice.wait_for(CopyAppWorkflowObjectIds.detailPanel_layout)
        self.spice.wait_for(CopyAppWorkflowObjectIds.vertical_layout_scrollbar)
        self.spice.wait_for(CopyAppWorkflowObjectIds.copy_settings_paper_selection_source)
        menu_item_id = [CopyAppWorkflowObjectIds.copy_settings_paper_selection_source, CopyAppWorkflowObjectIds.copy_settings_paper_selection_source_combobox]
        view_id = CopyAppWorkflowObjectIds.detailPanel_layout
        scroll_bar_id = CopyAppWorkflowObjectIds.vertical_layout_scrollbar
        self.workflow_common_operations.goto_item(menu_item_id, view_id, scrollbar_objectname = scroll_bar_id)

    def verify_copy_settings_option_constrained(self, option_name, net, constraints_unavilable = False):
        try:
            view_id = CopyAppWorkflowObjectIds.view_copySettingsView
            scroll_bar_id = CopyAppWorkflowObjectIds.copy_options_scrollbar
            if option_name == CopyAppWorkflowObjectIds.copy_sides_str_id :
                menu_item_id = [CopyAppWorkflowObjectIds.row_combo_copySettings_sides, CopyAppWorkflowObjectIds.combo_copySettings_sides]
            elif option_name == CopyAppWorkflowObjectIds.scan_colorMode_str_id :
                menu_item_id = [CopyAppWorkflowObjectIds.row_combo_copySettings_color, CopyAppWorkflowObjectIds.combo_copySettings_color]
            elif option_name == CopyAppWorkflowObjectIds.scan_contentType_str_id :
                menu_item_id = [CopyAppWorkflowObjectIds.row_combo_copySettings_contentType, CopyAppWorkflowObjectIds.combo_copySettings_contentType]
            elif option_name == CopyAppWorkflowObjectIds.copy_automatically_straighten_str_id : 
                menu_item_id = [CopyAppWorkflowObjectIds.settings_scan_auto_straighten, CopyAppWorkflowObjectIds.toggle_button_scan_auto_straighten+" MouseArea"]
            elif option_name == CopyAppWorkflowObjectIds.copy_resize_str_id :  
                menu_item_id = [CopyAppWorkflowObjectIds.list_copySettings_outputScale,CopyAppWorkflowObjectIds.list_copySettings_outputScale_value]
            elif option_name == CopyAppWorkflowObjectIds.copy_pagesPerSheet_str_id :  
                menu_item_id = [CopyAppWorkflowObjectIds.list_copySettings_pagesPerSheet, CopyAppWorkflowObjectIds.list_copySettings_pagesPerSheet_value]                 
            elif option_name == CopyAppWorkflowObjectIds.copy_output_bin_str_id :  
                menu_item_id = [CopyAppWorkflowObjectIds.row_output_destination_comboBox, CopyAppWorkflowObjectIds.output_destination_comboBox] 
            elif option_name == CopyAppWorkflowObjectIds.copy_booklet_str_id  : 
                menu_item_id = [CopyAppWorkflowObjectIds.list_copySettings_booklet, CopyAppWorkflowObjectIds.list_copySettings_booklet_value] 
            elif option_name == CopyAppWorkflowObjectIds.copy_scan_mode_str_id  : 
                menu_item_id = [CopyAppWorkflowObjectIds.scan_mode_settings_option, CopyAppWorkflowObjectIds.row_scan_mode_settings_option]  
            elif option_name == CopyAppWorkflowObjectIds.copy_paperSelection_str_id  : 
                menu_item_id = [CopyAppWorkflowObjectIds.list_copySettings_paperSelection, CopyAppWorkflowObjectIds.view_paperSetting]
            elif option_name == CopyAppWorkflowObjectIds.copy_content_orientation_str_id :   
                menu_item_id = [CopyAppWorkflowObjectIds.row_object_copy_orientation, CopyAppWorkflowObjectIds.settings_copy_orientation]
            elif option_name == CopyAppWorkflowObjectIds.copy_fold_str_id :   
                menu_item_id = [CopyAppWorkflowObjectIds.list_copy_fold, CopyAppWorkflowObjectIds.list_copy_fold_value] 
            elif option_name == CopyAppWorkflowObjectIds.copy_watermark_str_id :
                menu_item_id = [CopyAppWorkflowObjectIds.list_watermark, CopyAppWorkflowObjectIds.list_watermark_value]     
            elif option_name == CopyAppWorkflowObjectIds.copy_stamp_str_id :
                menu_item_id = [CopyAppWorkflowObjectIds.list_stamp_menu, CopyAppWorkflowObjectIds.list_stamp_value]      

            self.workflow_common_operations.goto_item(menu_item_id, view_id, scrollbar_objectname = scroll_bar_id)
            self.validate_constrained_message_on_settings_option(net,option_name,constraints_unavilable)
        finally:
            logging.info("Back to copy screen")
            self.back_to_landing_view()      
    
    def verify_slider_option_is_constrained(self, option_name):
        try:
            slider_name = None
            if option_name == CopyAppWorkflowObjectIds.copy_contrast_str_id :
                self.goto_contrast_option()
                slider_name = CopyAppWorkflowObjectIds.slider_contrast
            elif option_name == CopyAppWorkflowObjectIds.copy_sharpness_str_id :
                self.goto_sharpness_option()
                slider_name = CopyAppWorkflowObjectIds.slider_sharpness
            elif option_name == CopyAppWorkflowObjectIds.scan_lighterDarker_str_id :
                self.goto_lighter_darker_option()
                slider_name = CopyAppWorkflowObjectIds.slider_lighterDarker
            elif option_name == CopyAppWorkflowObjectIds.copy_background_cleanup_str_id :
                self.goto_background_cleanup_option()
                slider_name = CopyAppWorkflowObjectIds.slider_backGroundCleanup

            slider = self.spice.wait_for(slider_name)         
            self.spice.wait_until(lambda: slider["constrained"] == True)
        finally:
            logging.info("Back to copy screen")
            self.back_to_landing_view()
    ...
    def check_sides_property_locked(self, side_mode:str) -> bool:
        ...
        #to close combobox select 2-2 sided as this is available always
