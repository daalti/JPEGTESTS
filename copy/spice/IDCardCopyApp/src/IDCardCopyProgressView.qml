...
SystemProgress {
    id: idCardCopyingProgressView
    objectName: "IDCardCopyingProgress"
    ...
    StackView.onActivating: {
        console.log("Activating the ID_COPY_COPY_PROGRESS view, _stateMachine.jobInfoModel updated")
    }

    Timer{
        id: jobPollingTimeout
        objectName: "idCardCopyJobPollingTimeout"
        running: false
        interval: 1000
        repeat: true
        onTriggered: {
            console.log("IDCardCopyProgressView timeout: re-query the data")
    ...
    Component.onCompleted: {
        console.log("IDCardCopyProgressView onCompleted")

        jobPollingTimeout.running = true;

        //query job status.
        //Need to keep get till jobs/{jobId} support dynamic url subscription
        let jobInfoResponse = _resourceStore.get(_stateMachine.jobInfoModel.data.url);
