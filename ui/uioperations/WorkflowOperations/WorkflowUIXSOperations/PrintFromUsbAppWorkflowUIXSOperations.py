...
class PrintFromUsbAppWorkflowUIXSOperations(PrintFromUsbAppWorkflowUICommonOperations):
    ...
    def check_spec_on_usb_print_print_from_usb(self, net):
        """
        For XS screen machine, there is no string on continue button. Hence check the button is visible on Continue_PrintFromUSB
        The XS/S screen has a continue button instead of a print button.
        @param net:
        @return:
        """
        logging.info("check the button visible on Continue_PrintFromUSB screen")
        continue_button = self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.continue_button_locator)
        self.spice.validate_button(continue_button)
    ...
    def press_back_button_from_folder_view(self, screen_id, landing_view, index):
        if (self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.continue_button_locator)["visible"] != True):
            assert self.spice.query_item(PrintFromUsbAppWorkflowObjectIds.back_button, 1)["visible"] == True
            self.spice.query_item(PrintFromUsbAppWorkflowObjectIds.back_button, 1).mouse_click()
            # backButton = self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.back_button, timeout=15)
