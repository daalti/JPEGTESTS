...
class ToolsAppWorkflow2UISOperations(ToolsAppWorkflowUICommonOperations):
    ...
    def verify_toast_message(self, message):
        ...
        assert self._spice.wait_for(self.MenuAppWorkflowUISObjectIds.view_troubleshooting_fax_toastinfo,10)["text"] == message
    ...
    def setValue_and_save(self,buttonId,index,cdm,element_list):
        if index > 3 and index < len(element_list):
            Scroling_btn = self._spice.wait_for(MenuAppWorkflowObjectIds.scrollbar_fax_t30_report_mode)
            Scroling_btn.__setitem__("position", 0.26)
        ...
        self._spice.wait_for(MenuAppWorkflowObjectIds.view_troubleshooting_fax_t30_repmode_menu)
    ...
    def troubleshoot_fax_faxT30RepMode(self, cdm):
        ...
        for element in element_list:
            assert self.setValue_and_save(element[0],index,cdm,element_list)==element[1],"Set operation failed"
            index +=1
            sleep(1)
            if element[0] != element_list[-1][0]:
                self._spice.wait_for (MenuAppWorkflowObjectIds.view_troubleshooting_fax_t30_repmode_menu).mouse_click()
                sleep(2)
    ...
    def check_tray_alignment_menu(self, button_id):
        # Navigate to Tray alignment menu.
        self._spice.wait_for(self.MenuAppWorkflowUISObjectIds.menu_button_troubleshooting_print_quality_image_registration_tray).mouse_click()
        self._spice.wait_for(button_id).mouse_click()

    def check_tray_alignment_apply(self, cdm):
        ...
        # Navigate to Tray alignment menu
        scrollbar = self._spice.wait_for(MenuAppWorkflowObjectIds.scrollbar_tools_troubleshooting_printquality)
        scrollbar.__setitem__("position", 0.4)

        self._spice.wait_for(self.MenuAppWorkflowUISObjectIds.menu_button_troubleshooting_print_quality_image_registration).mouse_click()
        self._spice.wait_for(self.MenuAppWorkflowUISObjectIds.menu_button_troubleshooting_print_quality_image_registration_tray).mouse_click()
        ...
        for i in range(4):
            tray_obj = self._spice.wait_for(self.MenuAppWorkflowUISObjectIds.menu_button_troubleshooting_print_quality_image_registration_tray_spinbox + "%s" % (i+1))
        ...
        self._spice.wait_for(self.MenuAppWorkflowUISObjectIds.menu_button_apply).mouse_click()
        ...
        self._spice.wait_for(self.MenuAppWorkflowUISObjectIds.view_toast_window,10)
        assert self._spice.wait_for(self.MenuAppWorkflowUISObjectIds.view_troubleshooting_fax_toastinfo,10)["text"] == "Tray values saved"
        self._spice.query_item("#HomeButton #ButtonControl")
        sleep(2)

    def troubleshooting_print_quality_image_registration(self,cdm):
        # Verify Apply Button.
        self.check_tray_alignment_apply(cdm)
        
        # Verify closeButton in Tray alignment menu.
        self.check_tray_alignment_menu(self.MenuAppWorkflowUISObjectIds.menu_button_close)
 
        # Verify printButton in Tray alignment menu.
        self.check_tray_alignment_menu(self.MenuAppWorkflowUISObjectIds.menu_button_print_button)
        self.verify_toast_message("Printing...")
    ...
    def servicemenu_servicetests_serviceinfinitehs(self, kvp, cdm, job, net, locale):
        ...
        assert self._spice.wait_for("#" + kvp["view"])
