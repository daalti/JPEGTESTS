...
class PrintFromUsbAppWorkflowUIXLOperations(PrintFromUsbAppWorkflowUICommonOperations):
    ...
    def select_print_file_or_folder_by_name(self, name: str, dial_value: int = 180):
        ...
        rowObjectY = self.spice.wait_for(f"#{name}")["y"]
        if (rowObjectY != 0):
            assert self.spice.wait_for( PrintFromUsbAppWorkflowObjectIds.view_print_usb_folder_grid_view)["contentHeight"] != 0
            contentHeight = self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.view_print_usb_folder_grid_view)["contentHeight"]
    ...
    def check_search_result_number_in_folder_list_view(self, expected_file_list):
        ...
        time.sleep(5)
        self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.print_usb_folder_grid_view)
        self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.search_result_message_locator, timeout = 60.0)
    ...
    def check_spec_on_usb_print_options(self, net, two_sided=None,color=None, quality=None, paper_selection=None, collate=None):
        ...
        if(paper_selection is not None and paper_selection['paper_size'] and paper_selection['paper_type'] and paper_selection['paper_tray']):
    ...
    def set_copy_2sided_options(self, two_sided_options="off"):
        ...
        active_item = self.spice.query_item(PrintFromUsbAppWorkflowObjectIds.options_two_sided_menu_switch + " MouseArea", 1)
        #self.dial_common_operations.goto_item(PrintFromUsbAppWorkflowObjectIds.options_two_sided_menu_switch, PrintFromUsbAppWorkflowObjectIds.options_view,
        #                                      select_option=False)
    ...
    def set_color_options(self, net, color_options="color", locale: str = "en-US"):
        ...
        color_item = self.spice.query_item(PrintFromUsbAppWorkflowObjectIds.options_view + " " + PrintFromUsbAppWorkflowObjectIds.option_color_button_locator)
        ...
        if color_options == "grayscale":
            gray_item = self.spice.query_item(PrintFromUsbAppWorkflowObjectIds.option_color_set_grayscale_xl)
            gray_item.mouse_click()
            time.sleep(1)
    ...
    def set_collate_options(self, collate_options="off"):
        ...
        self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.options_view)
        collate_switch = self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.options_collate_menu_switch)
        
        if collate_options == "off" and collate_switch["checked"]:
            logging.info("Turn off collate option")
            collate_switch = self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.options_collate_menu_switch + " MouseArea")
            collate_switch.mouse_click()
            time.sleep(1)
        
        if collate_options == "on" and not collate_switch["checked"]:
            logging.info("Turn on collate option")
            collate_switch = self.spice.wait_for(PrintFromUsbAppWorkflowObjectIds.options_collate_menu_switch + " MouseArea")
            collate_switch.mouse_click()
            time.sleep(1)
    ...
    def goto_sort_filter_search_options_menu(self):
        ...
        #do nothing custom section is always there