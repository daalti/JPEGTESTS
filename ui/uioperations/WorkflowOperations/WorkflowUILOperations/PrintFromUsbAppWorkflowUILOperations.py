...
class PrintFromUsbAppWorkflowUILOperations(PrintFromUsbAppWorkflowUICommonOperations):
    ...
    def goto_print_app_homescreen(self):
        """
        Function to navigate to Print app on home screen
        Ui Flow: Any screen -> Home screen -> Print app
        @return:
        ...
        self.spice.wait_for(self.home_screen_view)
        scan_app = self.spice.wait_for(MenuAppWorkflowObjectIds.print_app_button + " MouseArea")
        scan_app.mouse_click()
        logging.info("At Home Screen Print from Usb App")
    ...
    def goto_print_from_usb_homescreen(self):
        """
        Purpose: navigate to Print from USB under Print app.
        Ui Flow: Home_Print -> USBPrint_NoFileFound/USBPrint_PrintFromUSBHome
        @return:
